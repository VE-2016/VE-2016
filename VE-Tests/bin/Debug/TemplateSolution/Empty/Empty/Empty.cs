...
namespace WinExplorer
{
    ...
    public partial class ExplorerForms : Form
        ...
        public ParserForm plf { get; set; }

        public void LoadWT(string name, bool load)
        {
            wt = new ToolWindow();
            wt.Text = name;
            wt.HideOnClose = true;
            wt.FormBorderStyle = FormBorderStyle.None;
            wt.TabText = name;
            wt.TopLevel = false;
            ...
            if (plf == null)
            {
                ...
                plf.ef = this;
                plf.TopLevel = false;
                plf.FormBorderStyle = FormBorderStyle.None;
                plf.Dock = DockStyle.Fill;
                plf.Show();
            }
            ...
            if (load)
                wt.Show(dock, DockState.DockBottom);
        }
        ...
        private void workerFunction(string recent)
        {
            ...
            this.Invoke(new Action(() =>
            {
                Loads2(tv, _sv._SolutionTreeView); this.Text = Path.GetFileName(recent) + " - Application Studio";
                if (scr != null)
                {
                    vs = GetVSSolution();
        ...
        public void rebuild_alls(string recent)
        {
            _sv.build_solution(recent, true);
        }

        public void build_alls(string recent)
        {
            _sv.build_solution(recent, false);
        }
        ...
        private void AddNewProject(bool addproject)
        {
            ...
            string tempfile = sln + "\\" + pn + "\\" + projs;

            string projectfile = sln + "\\" + pn + "\\" + proj;
        ...
        public void ViewHelpFiles()
        {
            ...
            // wf = sv.eo.OpenWebForm("Doxygen");

            string s = AppDomain.CurrentDomain.BaseDirectory;

            wf.wb.Navigate("file://" + s + "CHM\\index.html");
        ...
        private void outlineWindowToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                wt.Hide();
                outlineWindowToolStripMenuItem.Checked = false;
            }
            else
            {
                if (dock.Contains(plf) == false)
                    wt.Show(dock, DockState.DockBottom);
                wt.Show();
                outlineWindowToolStripMenuItem.Checked = true;
            }
        }
