...

{
    ...
    public class ClassMapper
    {
        ...
        /// The classcode.
        /// </value>
        public ArrayList baseclasses { get; set; }
        ...
        public MemberCreator addmembers(IMember m, ArrayList members)
        {
            if (members == null)
                members = new ArrayList();
        ...
        public MethodCreator addmethod(IMethod m, AstNode ast, bool isctor)
        {
            ...
            string ns = m.FullName.Replace("..ctor", "");

            string[] s = ns.Split(".".ToCharArray());
        ...
        public bool IsClassHeader(int Y)
        {
            if (start.Y == Y)
                return true;
        ...
        public PropertyCreator GetPropertyCreator(int Y)
        {
            if (properties == null)
                return null;
            ...
            {
                PropertyCreator c = properties[i] as PropertyCreator;
        ...
        public PropertyCreator GetPropertyCreator(string name)
        {
            if (properties == null)
                return null;
        ...
        public FieldCreator GetFieldCreator(string name)
        {
            if (fields == null)
                return null;
        ...
        public ArrayList GetPropertyNames()
        {
            ...
            if (properties != null)
                foreach (PropertyCreator f in properties)
                {
                    F.Add(f.InnerName);
                }
    ...
    public class ClassVisitor : DepthFirstAstVisitor
    {
        ...
        public override void VisitUsingDeclaration(UsingDeclaration usingStatement)
        {
            ReferenceCreator rc = new ReferenceCreator();

            if (mapper.references == null)
                mapper.references = new ArrayList();

            rc.name = usingStatement.Namespace;

            mapper.references.Add(rc);

            base.VisitUsingDeclaration(usingStatement);
        }
        ...
        public override void VisitTypeDeclaration(TypeDeclaration typeDeclaration)
        {
            ...
            ArrayList refs = mapper.references;

            mapper = new ClassMapper();

            mapper.references = refs;

            mappers.Add(mapper);
            //mapper.HasBeenCreated = false;

            //if (mapper.HasBeenCreated == )
            //{
            // //   if (mappers == null)
            // //       mappers = new ArrayList();
            //    mappers.Add(mapper);
            ...
            mapper.baseclasses = new ArrayList();

            foreach (AstNode node in typeDeclaration.BaseTypes)
            {
                string s = node.ToString();
                mapper.baseclasses.Add(s);
            }
            ...
            mapper.start = new Point(typeDeclaration.NameToken.StartLocation.Column, typeDeclaration.NameToken.StartLocation.Line);

            mapper.end = new Point(typeDeclaration.NameToken.EndLocation.Column, typeDeclaration.NameToken.EndLocation.Line);
            ...
            //IEnumerable<IType> bc = typeResolveResult.Type.DirectBaseTypes;

            //foreach (IType ts in bc)
            //{
            //                   if (ts.Name == mapper.classname)
            //        continue;

            //    ArrayList members = new ArrayList();

            //    IEnumerable<IMember> m = ts.GetMembers();

            //    foreach (IMember member in m)
            //    {
            //        MemberCreator fc = mapper.addmembers(member, members);

            //        fc.DeclaringType = ts.Name;
            //    }

            //    mapper.baseclasses.Add(members);
            //}
            ...
            foreach (IMethod member in typeResolveResult.Type.GetConstructors(constructor => true))
            {
                AstNode ast = syntax.GetNodeContaining(member.Region.Begin, member.Region.End);
    ...
    public class MainTest
    {
        ...
        static public ClassMapper analysecode(string sourceText, string file)
        {
            ...
            try
            {
                syntaxTree = parser.Parse(sourceText, file);
            }
            catch (Exception e) { };
        ...
        static public void AnalyzeSyntaxTree(SyntaxTree syntax, ClassMapper mapperw)
        {
            if (mapperw == null)
                return;

            if (mapperw.mappers == null)
                return;

            foreach (ClassMapper mapper in mapperw.mappers)
            {
                ...
                foreach (EntityDeclaration ent in doc)
                {
                    ...
                    foreach (AstNode ast in asc.Descendants)
                    {
                        if (ast.NodeType.ToString() == "MemberType" || ast.NodeType.ToString() == "Member")
                        {
                            ...
                            if (ast.GetType() == typeof(MethodDeclaration) || T == typeof(ConstructorDeclaration))
                            {
    ...
    public class Parser
    {
        static public void CreateProjects(TreeNode tvs, Dictionary<string, ClassMapper> dict, bool parser)
        {
            ...
            foreach (string key in maps.Keys)
            {
                ...
                if (ns == null || ns.Length <= 0)
                {
                    ...
                    node.Text = project;
                    node.Name = project;
                    node.ImageKey = "Namespace";// NodeImages.Keys.AssemblyImage;
                ...
                if (ng == null || ng.Length <= 0)
                {
                    ...
                    nodes.ImageKey = "Namespace";// NodeImages.Keys.AssemblyImage;
                    nodes.SelectedImageKey = "Namespace";

                    TreeNode b = new TreeNode();
                    b.Text = m.classname;
                    b.Name = m.classname;
                    b.ImageKey = "Class_489_24";
                    b.SelectedImageKey = "Class_489_24";
                    ArrayList L = null;// GetTypeMembers(m);
                    b.Tag = L;// m;
        ...
        //static public ArrayList GetTypeMembers(ClassMapper m)
        //{
        //    ArrayList L = new ArrayList();
        //    if (m.methods != null)
        //        foreach (MethodCreator c in m.methods)
        //        {
        //            TreeNode node = new TreeNode();
        //            node.Text = c.InnerName;
        //            node.ImageKey = AndersLiu.Reflector.Program.UI.AssemblyTreeNode.NodeImages.Keys.MethodImage;
        //            node.SelectedImageKey = AndersLiu.Reflector.Program.UI.AssemblyTreeNode.NodeImages.Keys.MethodImage;

        //            L.Add(node);
        //        }
        //    if (m.fields != null)
        //        foreach (FieldCreator c in m.fields)
        //        {
        //            TreeNode node = new TreeNode();
        //            node.Text = c.InnerName;
        //            node.ImageKey = AndersLiu.Reflector.Program.UI.AssemblyTreeNode.NodeImages.Keys.FieldImage;
        //            node.SelectedImageKey = AndersLiu.Reflector.Program.UI.AssemblyTreeNode.NodeImages.Keys.FieldImage;

        //            L.Add(node);
        //        }
        //    if (m.properties != null)
        //        foreach (PropertyCreator c in m.properties)
        //        {
        //            TreeNode node = new TreeNode();
        //            node.Text = c.InnerName;
        //            node.ImageKey = AndersLiu.Reflector.Program.UI.AssemblyTreeNode.NodeImages.Keys.PropertyImage;
        //            node.SelectedImageKey = AndersLiu.Reflector.Program.UI.AssemblyTreeNode.NodeImages.Keys.PropertyImage;
        ...
        static public void BuildMapsNode(VSProject pp, TreeNode node, Dictionary<string, ClassMapper> dict)
        {
            ...
            CreateProjects(node, dict, false);
        }

        static public void BuildMaps(VSProject p, TreeNode node)
        {
            Dictionary<string, ClassMapper> maps = new Dictionary<string, ClassMapper>();

            BuildMapsNode(p, node, maps);

            CreateProjects(node, maps, true);
        }
    }
}