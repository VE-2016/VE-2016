...
namespace VSParsers
{
    public class ProjectItemInfo : object
    {
        ...
        [Category("File")]
        public EntityDeclaration/*ClassMapper*/ mapper { get; set; }
        ...
        public static string FileNameIfAny(TreeNode node)
        {
            if (node.Tag == null)
                return "";
        ...
        public static string FileNameIfAny(ProjectItemInfo p)
        {
            var pr0 = p as VSParsers.ProjectItemInfo;
        ...
        public static VSProject VSProjectIfAny(TreeNode node)
        {
            if (node.Tag == null)
                return null;
        ...
        public static ProjectItemInfo ToProjectItemInfo(TreeView v, string file)
        {
            foreach (TreeNode node in v.Nodes)
            {
                var ns = ToProjectItemInfo(node, file);
                if (ns != null)
                    return ns;
            }
            return null;
        ...
        public static ProjectItemInfo ToProjectItemInfo(TreeNode node, string file)
        {
            ProjectItemInfo p = node.Tag as ProjectItemInfo;
            if (p != null)
            {
                string filename = FileNameIfAny(p);
                if (filename != null)
                {
                    if (filename.ToLower() == file)
                        return p;
                }
            }
            file = file.ToLower();
            ...
            {
                var ns = ToProjectItemInfo(nodes, file);
                if (ns != null)
                    return ns;
            }
            return null;
        ...
        public static ProjectItemInfo ToProjectItemInfo(TreeNode node)
        {
            if (node.Tag == null)
                return null;
        ...
        public VSProject VSProjectIfAny()
        {
            return ps;
        }
    ...
    public class msbuilder_alls
    {
        ...
        public static TreeView BuidlSolutionAndFolders(string file)
        {
            TreeView v = new TreeView();

            string folder = Path.GetDirectoryName(file);

            var img = VSSolutionLoader.CreateImageList(AppDomain.CurrentDomain.BaseDirectory);

            var dc = VSSolutionLoader.FileExtensions();
            ...
            if (directoryInfo.Exists)
                BuildTree(directoryInfo, v.Nodes);
        ...
        public static void BuildTree(DirectoryInfo directoryInfo, TreeNodeCollection addInMe)
        {
            ...
            curNode.SelectedImageKey = "FolderOpem";
            ...
            {
                var ng = curNode.Nodes.Add(file.FullName, file.Name);
                ng.ImageKey = "Folder";
                ng.SelectedImageKey = "FolderOpem";
            }
        }
        ...
        public VSSolution tester(TreeView tv, string paths, string thisproject)
        {
            ...
            vs.SolutionParsed = solution as VSProvider.Solution;
            ...
            foreach (VSProjectItem pi in CDD)
            {
                ...
                foreach (string s in cc)
                {
                    ...
                    if (ns == null)
                    {
                        ...
                        if (i < cc.Length - 1)
                        {
                            ns.SelectedImageKey = "FolderOpen";
                            ns.ImageKey = "Folder";
                        }
                        nodes.Nodes.Add(ns);
                    }
        ...
        public static void SetExtensions(TreeView v, Dictionary<string, string> dc)
        {
            foreach (TreeNode node in v.Nodes)
                SetExtensions(node, dc);
        }

        public static void SetExtensions(TreeNode node, Dictionary<string, string> dc)
        {
            string[] gg = node.Text.Split("\\".ToCharArray());
    ...
    public class VSExplorer
    {
        ...
        static public ImageList CreateImageList(string s)
        {
            ...
            if (!File.Exists(s + "\\" + "WinExplorer.exe"))
                throw new FileNotFoundException("File does not exists - " + s + "\\" + "WinExplorer.exe");
    ...
    public class VSSolutionLoader
    {
        ...
        private ImageList imageList1 { get; set; }
        ...
        public TreeView LoadProjectFolder(string sln)
        {
            string s = AppDomain.CurrentDomain.BaseDirectory;
        ...
        static public ImageList addImage(Bitmap bmp, string name, ImageList imgL)
        {
            ...
            if (bmp != null)
            {
                ...
                imgL.Images.Add(name, (Image)new Bitmap(bmp));
            }
        ...
        static public ImageList CreateImageList(string s)
        {
            ...
            if (dict == null)
            {
                dict = Load(s + "\\" + "VE-Resources.dll", "WinExplorers.ve.resources");
            ...
            if (dict3 == null)
            {
                addImage(ve.VSO_Folder_16x, "Folder", imageList1);
                addImage(ve.FolderOpen_16x, "FolderOpen", imageList1);
                ...
                addImage(ve.LockCyan_16x, "LockCyan_16x", imageList1);
                addImage(ve.Add_thin_10x_16x, "Add_thin", imageList1);
                addImage(ve.StatusOK_16xSM, "Status_OK", imageList1);
                addImage(ve.StatusOffline_16xSM, "Status_Offline", imageList1);
                addImage(ve.WindowsForm_256x, "forms", imageList1);
                addImage(ve.Control_16x, "control", imageList1);
                addImage(ve.Component_16x, "component", imageList1);
                addImage(ve.UserControl_16x, "usercontrol", imageList1);
            }
        ...
        public async void CompileSolution(string solutionFileName, VSSolution vs)
        {
            //Stopwatch sw = Stopwatch.StartNew();

            vs.SetAutoResetEvent(false);
            ...
            foreach (ProjectId projectId in projectGraph.GetTopologicallySortedProjects())
            {
                ...
                vs.SetAutoResetEventForProjectName(project.Name, true);
            }
            //sw.Stop();
