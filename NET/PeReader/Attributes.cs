...
namespace Microsoft.Cci.MetadataReader.ObjectModelImplementation
{
    ...
    {
        private readonly IModuleTypeReference _typeReference;
        ...
        #endregion ICompileTimeConstant Members
    }
    ...
    internal sealed class ArrayExpression : ExpressionBase, IMetadataCreateArray
    {
        ...
        #endregion IArrayCreate Members
    }

    internal sealed class TypeOfExpression : ExpressionBase, IMetadataTypeOf
    {
        private readonly PEFileToObjectModel _PEFileToObjectModel;
        private readonly IModuleTypeReference/*?*/ _typeExpression;
        ...
        {
            _PEFileToObjectModel = peFileToObjectModel;
            _typeExpression = typeExpression;
        }

        internal override IModuleTypeReference/*?*/ ModuleTypeReference
        {
            get { return _PEFileToObjectModel.SystemType; }
        }
        ...
        public ITypeReference TypeToGet
        {
            get
            {
                if (_typeExpression == null) return Dummy.TypeReference;
                return _typeExpression;
            }
        }

        #endregion ITypeOf Members
    }

    internal sealed class FieldOrPropertyNamedArgumentExpression : ExpressionBase, IMetadataNamedArgument
    {
        private const int IsFieldFlag = 0x01;
        private const int IsResolvedFlag = 0x02;
        private readonly IName _name;
        private readonly ITypeReference _containingType;
        private int _flags;
        private readonly IModuleTypeReference _fieldOrPropTypeReference;
        private object/*?*/ _resolvedFieldOrProperty;
        ...
        )
        {
            ...
            if (isField)
                _flags |= FieldOrPropertyNamedArgumentExpression.IsFieldFlag;
            _fieldOrPropTypeReference = fieldOrPropTypeReference;
        ...
        public bool IsField
        {
            get
            {
                return (_flags & FieldOrPropertyNamedArgumentExpression.IsFieldFlag) == FieldOrPropertyNamedArgumentExpression.IsFieldFlag;
            }
        }

        internal override IModuleTypeReference/*?*/ ModuleTypeReference
        {
            get { return _fieldOrPropTypeReference; }
        }
        ...
        public object/*?*/ ResolvedDefinition
        {
            get
            {
                if ((_flags & FieldOrPropertyNamedArgumentExpression.IsResolvedFlag) == 0)
                {
                    _flags |= FieldOrPropertyNamedArgumentExpression.IsResolvedFlag;
                    ITypeDefinition/*?*/ typeDef = _containingType.ResolvedType;
                    if (this.IsField)
                    {
                        foreach (ITypeDefinitionMember tdm in typeDef.GetMembersNamed(_name, false))
                        {
                            ...
                            if (fmtr == null)
                                continue;
                            if (fmtr.InternedKey == _fieldOrPropTypeReference.InternedKey)
                            {
                                _resolvedFieldOrProperty = fd;
                                break;
                            }
                        }
                    }
                    else
                    {
                        foreach (ITypeDefinitionMember tdm in typeDef.GetMembersNamed(_name, false))
                        {
                            ...
                            if (pmtr == null)
                                continue;
                            if (pmtr.InternedKey == _fieldOrPropTypeReference.InternedKey)
                            {
                                _resolvedFieldOrProperty = pd;
                                break;
                            }
                        }
                    }
                }
                return _resolvedFieldOrProperty;
            }
        }

        #endregion INamedArgument Members
    }
    ...
    internal sealed class SecurityAttribute : MetadataObject, ISecurityAttribute
    {
        ...
        private EnumerableArrayWrapper<SecurityCustomAttribute, ICustomAttribute>/*?*/ _securityCustomAttributes;
        ...
        IEnumerable<ICustomAttribute> ISecurityAttribute.Attributes
        {
            get
            {
                if (_securityCustomAttributes == null)
                {
                    _securityCustomAttributes = this.PEFileToObjectModel.GetSecurityAttributeData(this);
                }
                return _securityCustomAttributes;
            }
        }
    ...
    internal sealed class NamespaceTypeName : NominalTypeName
    {
        ...
        )
        {
            ...
            string nameStr = null;
            TypeCache.SplitMangledTypeName(name.Value, out nameStr, out _genericParameterCount);
    ...
    internal sealed class TypeNameParser
    {
        ...
        private readonly IName _version;
        private readonly IName _publicKeyToken;
        private readonly IName _culture;
        private readonly IName _neutral;
        private int _currentIndex;
        private TypeNameTokenKind _currentTypeNameTokenKind;
        private IName _currentIdentifierInfo;

        private ScannerState ScannerSnapshot()
        {
            return new ScannerState(
              _currentIndex,
              _currentTypeNameTokenKind,
              _currentIdentifierInfo
            );
        }

        private void RestoreScanner(
          ScannerState scannerState
        )
        {
            _currentIndex = scannerState.CurrentIndex;
            _currentTypeNameTokenKind = scannerState.CurrentTypeNameTokenKind;
            _currentIdentifierInfo = scannerState.CurrentIdentifierInfo;
        }

        private void SkipSpaces()
        {
            int currPtr = _currentIndex;
            string name = _typeName;
            while (currPtr < _length && char.IsWhiteSpace(name[currPtr]))
            {
                currPtr++;
            }
            _currentIndex = currPtr;
        }

        private static bool IsEndofIdentifier(
          char c,
          bool assemblyName
        )
        {
        ...
        private byte[] ScanPublicKeyToken()
        {
            ...
            int currPtr = _currentIndex;
            string name = _typeName;
            if (currPtr + 4 <= _length && string.Compare(name.Substring(currPtr, 4), "null", StringComparison.OrdinalIgnoreCase) == 0)
            {
                _currentIndex += 4;
                return TypeCache.EmptyByteArray;
            }
            if (currPtr + 16 > _length)
            {
                return TypeCache.EmptyByteArray;
            }
        ...
        {
            this.SkipSpaces();
            if (_currentIndex >= _typeName.Length)
            {
                _currentTypeNameTokenKind = TypeNameTokenKind.EOS;
                return;
            }
            switch (_typeName[_currentIndex])
            {
                case '[':
                    _currentTypeNameTokenKind = TypeNameTokenKind.OpenBracket;
                    _currentIndex++;
                    break;

                case ']':
                    _currentTypeNameTokenKind = TypeNameTokenKind.CloseBracket;
                    _currentIndex++;
                    break;

                case '*':
                    _currentTypeNameTokenKind = TypeNameTokenKind.Astrix;
                    _currentIndex++;
                    break;

                case '.':
                    _currentTypeNameTokenKind = TypeNameTokenKind.Dot;
                    _currentIndex++;
                    break;

                case '+':
                    _currentTypeNameTokenKind = TypeNameTokenKind.Plus;
                    _currentIndex++;
                    break;

                case ',':
                    _currentTypeNameTokenKind = TypeNameTokenKind.Comma;
                    _currentIndex++;
                    break;

                case '&':
                    _currentTypeNameTokenKind = TypeNameTokenKind.Ampersand;
                    _currentIndex++;
                    break;
                ...
                case '=':
                    if (assemblyName)
                    {
                        _currentTypeNameTokenKind = TypeNameTokenKind.Equals;
                        _currentIndex++;
                        break;
                    }
                    goto default;
                default:
                    {
                        int currIndex = _currentIndex;
                        StringBuilder sb = new StringBuilder();
                        string name = _typeName;
                        while (currIndex < _length)
                        {
                        ...
                        _currentIdentifierInfo = _nameTable.GetNameFor(sb.ToString());
                        _currentTypeNameTokenKind = TypeNameTokenKind.Identifier;
                        break;
                    }
            }
        }

        private static bool IsTypeNameStart(
        ...
        )
        {
            ...
            _typeName = typeName;
            _length = typeName.Length;
            _version = nameTable.GetNameFor("Version");
            _publicKeyToken = nameTable.GetNameFor("PublicKeyToken");
            _culture = nameTable.GetNameFor("Culture");
            _neutral = nameTable.GetNameFor("neutral");
            _currentIdentifierInfo = nameTable.EmptyName;
            //^ base();
        ...
        {
            if (_currentTypeNameTokenKind != TypeNameTokenKind.Identifier)
            {
                return null;
            }
            IName lastName = _currentIdentifierInfo;
            NamespaceName/*?*/ currNsp = null;
            this.NextToken(false);
            while (_currentTypeNameTokenKind == TypeNameTokenKind.Dot)
            {
                this.NextToken(false);
                if (_currentTypeNameTokenKind != TypeNameTokenKind.Identifier)
                {
                ...
                currNsp = new NamespaceName(_nameTable, currNsp, lastName);
                lastName = _currentIdentifierInfo;
                this.NextToken(false);
            }
            return new NamespaceTypeName(_nameTable, currNsp, lastName);
        }

        private TypeName/*?*/ ParseGenericTypeArgument()
        {
            if (_currentTypeNameTokenKind == TypeNameTokenKind.OpenBracket)
            {
                this.NextToken(false);
                ...
                if (_currentTypeNameTokenKind != TypeNameTokenKind.CloseBracket)
                {
                    return null;
                }
                this.NextToken(false);
        ...
        private NominalTypeName/*?*/ ParseNominalTypeName()
        {
            ...
            if (nomTypeName == null)
                return null;
            while (_currentTypeNameTokenKind == TypeNameTokenKind.Plus)
            {
                this.NextToken(false);
                if (_currentTypeNameTokenKind != TypeNameTokenKind.Identifier)
                {
                ...
                nomTypeName = new NestedTypeName(_nameTable, nomTypeName, _currentIdentifierInfo);
                this.NextToken(false);
            }
            return nomTypeName;
        }
        ...
        private TypeName/*?*/ ParsePossiblyGenericTypeName()
        {
            ...
            if (_currentTypeNameTokenKind == TypeNameTokenKind.OpenBracket)
            {
                ...
                this.NextToken(false);
                if (TypeNameParser.IsTypeNameStart(_currentTypeNameTokenKind))
                {
                    ...
                    while (_currentTypeNameTokenKind == TypeNameTokenKind.Comma)
                    {
                        this.NextToken(false);
        ...
        private TypeName/*?*/ ParseFullName()
        {
            ...
            for (;;)
            {
                if (_currentTypeNameTokenKind == TypeNameTokenKind.Astrix)
                {
                    this.NextToken(false);
                    typeName = new PointerTypeName(typeName);
                }
                else if (_currentTypeNameTokenKind == TypeNameTokenKind.OpenBracket)
                {
                    this.NextToken(false);
                    int rank;
                    if (_currentTypeNameTokenKind == TypeNameTokenKind.Astrix)
                    ...
                    else if (_currentTypeNameTokenKind == TypeNameTokenKind.Comma)
                    {
                        rank = 1;
                        while (_currentTypeNameTokenKind == TypeNameTokenKind.Comma)
                        {
                    ...
                    else if (_currentTypeNameTokenKind == TypeNameTokenKind.CloseBracket)
                    {
                        rank = 0; // SZArray Case
                    }
                    else
            ...
            if (_currentTypeNameTokenKind == TypeNameTokenKind.Ampersand)
            {
                this.NextToken(false);
        ...
        private AssemblyIdentity/*?*/ ParseAssemblyName()
        {
            if (_currentTypeNameTokenKind != TypeNameTokenKind.Identifier)
            {
                return null;
            }
            IName assemblyName = _currentIdentifierInfo;
            this.NextToken(true);
            ...
            IName culture = _nameTable.EmptyName;
            while (_currentTypeNameTokenKind == TypeNameTokenKind.Comma)
            {
                this.NextToken(true);
                if (_currentTypeNameTokenKind != TypeNameTokenKind.Identifier)
                    return null;
                IName infoIdent = _currentIdentifierInfo;
                this.NextToken(true);
                if (_currentTypeNameTokenKind != TypeNameTokenKind.Equals)
                    return null;
                if (infoIdent.UniqueKeyIgnoringCase == _culture.UniqueKeyIgnoringCase)
                {
                    this.NextToken(true);
                    if (cultureRead || _currentTypeNameTokenKind != TypeNameTokenKind.Identifier)
                        return null;
                    culture = _currentIdentifierInfo;
                    if (culture.UniqueKeyIgnoringCase == _neutral.UniqueKeyIgnoringCase)
                        culture = _nameTable.EmptyName;
                    cultureRead = true;
                }
                else if (infoIdent.UniqueKeyIgnoringCase == _version.UniqueKeyIgnoringCase)
                {
                    if (versionRead)
                        return null;
                ...
                else if (infoIdent.UniqueKeyIgnoringCase == _publicKeyToken.UniqueKeyIgnoringCase)
                {
                    if (pkTokenRead)
                        return null;
                ...
                else
                {
                    ...
                    while (_currentTypeNameTokenKind != TypeNameTokenKind.Comma && _currentTypeNameTokenKind != TypeNameTokenKind.CloseBracket && _currentTypeNameTokenKind != TypeNameTokenKind.EOS)
                    {
                        this.NextToken(true);
                    }
                }
        ...
        private TypeName/*?*/ ParseTypeNameWithPossibleAssemblyName()
        {
            ...
            if (tn == null)
                return null;
            if (_currentTypeNameTokenKind == TypeNameTokenKind.Comma)
            {
                this.NextToken(true);
        ...
        internal TypeName/*?*/ ParseTypeName()
        {
            ...
            if (tn == null || _currentTypeNameTokenKind != TypeNameTokenKind.EOS)
                return null;
            return tn;
        }
    }
...
namespace Microsoft.Cci.MetadataReader
{
    internal abstract class AttributeDecoder
    {
        ...
        private bool _useByteForUnderlyingTypeOfUnresolvedEnum;
            ...
            switch (type.SignatureTypeCode)
            {
                ...
                default:
                    ...
                    else
                    {
                        ...
                        if (_useByteForUnderlyingTypeOfUnresolvedEnum) //have tried int32 and failed, now try byte
        ...
        )
        {
            ...
            _useByteForUnderlyingTypeOfUnresolvedEnum = useByteForUnderlyingTypeOfUnresolvedEnum;
        }
    }
