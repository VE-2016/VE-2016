...
namespace Microsoft.Cci.MetadataReader
{
    ...
    internal sealed class PEFileToObjectModel
    {
        ...
        private readonly Hashtable<IName> _stringIndexToNameTable;
        private readonly Hashtable<IName> _stringIndexToUnmangledNameTable;
        ...
        //^ requires !(moduleIdentity.Location != null && moduleIdentity.Location.Length != 0);
        {
            ...
            _stringIndexToNameTable = new Hashtable<IName>();
            _stringIndexToUnmangledNameTable = new Hashtable<IName>();
            ...
            _namespaceINameHashtable = new Hashtable<Namespace>();
            this.LoadNamespaces();
            _namespaceReferenceINameHashtable = new DoubleHashtable<NamespaceReference>();
            _namespaceTypeTokenTable = new DoubleHashtable(peFileReader.TypeDefTable.NumberOfRows + peFileReader.ExportedTypeTable.NumberOfRows);
            _nestedTypeTokenTable = new DoubleHashtable(peFileReader.NestedClassTable.NumberOfRows + peFileReader.ExportedTypeTable.NumberOfRows);
            this.PreLoadTypeDefTableLookup();
            _moduleTypeDefArray = new TypeBase/*?*/[peFileReader.TypeDefTable.NumberOfRows + 1];
            _moduleTypeDefLoadState = new LoadState[peFileReader.TypeDefTable.NumberOfRows + 1];
            _moduleTypeDefLoadState[0] = LoadState.Loaded;

            _exportedTypeArray = new ExportedTypeAliasBase/*?*/[peFileReader.ExportedTypeTable.NumberOfRows + 1];
            _exportedTypeLoadState = new LoadState[peFileReader.ExportedTypeTable.NumberOfRows + 1];
            _exportedTypeLoadState[0] = LoadState.Loaded;

            _moduleGenericParamArray = new GenericParameter[peFileReader.GenericParamTable.NumberOfRows + 1];
            if (peFileReader.MethodSpecTable.NumberOfRows > 0)
            {
                _moduleMethodSpecHashtable = new DoubleHashtable<GenericMethodInstanceReference>(peFileReader.MethodSpecTable.NumberOfRows + 1);
            }

            _moduleTypeRefReferenceArray = new TypeRefReference[peFileReader.TypeRefTable.NumberOfRows + 1];
            _moduleTypeRefReferenceLoadState = new LoadState[peFileReader.TypeRefTable.NumberOfRows + 1];
            _moduleTypeRefReferenceLoadState[0] = LoadState.Loaded;
            if (peFileReader.TypeSpecTable.NumberOfRows > 0)
            {
                _moduleTypeSpecHashtable = new DoubleHashtable<TypeSpecReference>(peFileReader.TypeSpecTable.NumberOfRows + 1);
            }

            _moduleFieldArray = new FieldDefinition[peFileReader.FieldTable.NumberOfRows + 1];
            _moduleMethodArray = new MethodDefinition[peFileReader.MethodTable.NumberOfRows + 1];
            _moduleEventArray = new EventDefinition[peFileReader.EventTable.NumberOfRows + 1];
            _modulePropertyArray = new PropertyDefinition[peFileReader.PropertyTable.NumberOfRows + 1];

            _moduleMemberReferenceArray = new MemberReference/*?*/[peFileReader.MemberRefTable.NumberOfRows + 1];

            _customAttributeArray = new ICustomAttribute/*?*/[peFileReader.CustomAttributeTable.NumberOfRows + 1];

            _declSecurityArray = new ISecurityAttribute/*?*/[peFileReader.DeclSecurityTable.NumberOfRows + 1];

            uint moduleClassTypeDefToken = _namespaceTypeTokenTable.Find((uint)this.NameTable.EmptyName.UniqueKey, (uint)peReader._Module_.UniqueKey);
        ...
        private AssemblyIdentity GetContractAssemblySymbolicIdentity()
        {
            lock (GlobalLock.LockingObject)
            {
                ...
                for (uint i = 1; i <= trTable.NumberOfRows; i++)
                {
                    ...
                    return _assemblyReferenceArray[resolutionScopeRowId].AssemblyIdentity;
                }
            ...
            lock (GlobalLock.LockingObject)
            {
                ...
                for (uint i = 1; i <= trTable.NumberOfRows; i++)
                {
                    ...
                    if (tKey != objectKey && tKey != valueTypeKey && tKey != enumKey && tKey != multicastDelegateKey && tKey != arrayKey &&
                      ...
                      tKey != runtimeTypeHandleKey && tKey != argIteratorKey && tKey != voidKey)
                        continue;
                    uint resolutionScopeKind = (tr.ResolutionScope & TokenTypeIds.TokenTypeMask);
                    if (resolutionScopeKind != TokenTypeIds.AssemblyRef) continue;
                    uint resolutionScopeRowId = (tr.ResolutionScope & TokenTypeIds.RIDMask);
                ...
                for (uint i = 1; i <= arTable.NumberOfRows; i++)
                {
                    ...
                    return _assemblyReferenceArray[i].AssemblyIdentity;
                }
            }
            return Dummy.Assembly.AssemblyIdentity;
        }
        ...
        {
            IName/*?*/ name = _stringIndexToNameTable.Find(offset);
            if (name == null)
            {
                string nameStr = this.PEFileReader.StringStream[offset];
                name = this.NameTable.GetNameFor(nameStr);
                _stringIndexToNameTable.Add(offset, name);
            }
            return name;
        }

        //  Caller should lock this.
        private IName GetUnmangledNameFromOffset(
          uint offset
        )
        {
            IName/*?*/ name = _stringIndexToUnmangledNameTable.Find(offset);
            if (name == null)
            {
                string nameStr = this.PEFileReader.StringStream[offset];
                ...
                name = this.NameTable.GetNameFor(nameStr);
                _stringIndexToUnmangledNameTable.Add(offset, name);
            }
            return name;
        }
        ...
        /// </summary>
        private AssemblyReference[] _assemblyReferenceArray;

        /// <summary>
        /// Cache for module references. This indexes from row id to module reference.
        /// </summary>
        private ModuleReference[] _moduleReferenceArray;
        ...
        //^ ensures this.AssemblyReferenceArray != null;
        {
            ...
            _assemblyReferenceArray = assemblyRefList;
        }
        ...
        //^ ensures this.ModuleReferenceArray != null;
        {
            ...
            _moduleReferenceArray = moduleRefList;
        }
        ...
        {
            if (assemRefRowId < _assemblyReferenceArray.Length)
            {
                return _assemblyReferenceArray[assemRefRowId];
            }
            return null;
        }
        ...
        {
            if (moduleRefRowId < _moduleReferenceArray.Length)
            {
                return _moduleReferenceArray[moduleRefRowId];
            }
            return null;
        }
        ...
        {
            AssemblyReference[] assemRefList = _assemblyReferenceArray;
        ...
        {
            ModuleReference[] moduleRefList = _moduleReferenceArray;
        ...
        internal AssemblyReference/*?*/ FindAssemblyReference(AssemblyIdentity assemblyIdentity)
        {
            ...
            AssemblyReference[] assemblyRefList = _assemblyReferenceArray;
        ...
        /// </summary>
        private FileReference[]/*?*/ _fileReferenceArray;

        /// <summary>
        /// Cache for Resource referneces of the assembly.
        /// </summary>
        private ResourceReference[]/*?*/ _resourceReferenceArray;
        ...
        //^ ensures this.FileReferenceArray != null;
        {
            lock (GlobalLock.LockingObject)
            {
                if (_fileReferenceArray == null)
                {
                    ...
                    _fileReferenceArray = fileReferenceArray;
                }
            }
        }
        ...
        )
        {
            ...
            if (fileRefRowId < _fileReferenceArray.Length)
            {
                return _fileReferenceArray[fileRefRowId];
            }
            return null;
        }
        ...
        internal IEnumerable<IFileReference> GetFiles()
        {
            ...
            //^ assert this.FileReferenceArray != null;
            FileReference[] fileRefList = _fileReferenceArray;
        ...
        //^ ensures this.ResourceReferenceArray != null;
        {
            lock (GlobalLock.LockingObject)
            {
                if (_resourceReferenceArray == null)
                {
                    ...
                    _resourceReferenceArray = resourceReferenceArray;
                }
            }
        }
        ...
        )
        {
            ...
            int num = _resourceReferenceArray.Length;
            for (int i = 1; i <= num; ++i)
            {
                if (_resourceReferenceArray[i].Name.UniqueKey == uniKey)
                {
                    return _resourceReferenceArray[i];
                }
            }
            return null;
        }
        ...
        internal IEnumerable<IResourceReference> GetResources()
        {
            ...
            //^ assert this.ResourceReferenceArray != null;
            ResourceReference[] resRefArr = _resourceReferenceArray;
        ...
        private readonly Hashtable<Namespace> _namespaceINameHashtable;
        private readonly DoubleHashtable<NamespaceReference> _namespaceReferenceINameHashtable;

        //  Loaded by the constructor when this object is created
        private Namespace GetNamespaceForString(
          string namespaceFullName
        )
        {
            IName iNamespaceFullName = this.NameTable.GetNameFor(namespaceFullName);
            Namespace/*?*/ retNamespace = _namespaceINameHashtable.Find((uint)iNamespaceFullName.UniqueKey);
            ...
            _namespaceINameHashtable.Add((uint)iNamespaceFullName.UniqueKey, retNamespace);
            return retNamespace;
        }

        private void BuildNamespaceForNameOffset(
        ...
        {
            _namespaceINameHashtable.Add((uint)NameTable.EmptyName.UniqueKey, this.RootModuleNamespace);
        ...
        private Namespace GetNamespaceForNameOffset(
          uint namespaceNameOffset
        )
        {
        ...
        {
            NamespaceReference/*?*/ retNamespaceReference = _namespaceReferenceINameHashtable.Find(moduleReference.InternedModuleId, (uint)iNamespaceFullName.UniqueKey);
            ...
            _namespaceReferenceINameHashtable.Add(moduleReference.InternedModuleId, (uint)iNamespaceFullName.UniqueKey, retNamespaceReference);
            return retNamespaceReference;
        }
        ...
        private readonly DoubleHashtable _namespaceTypeTokenTable;

        //  <ParentTypeToken(Def/Exported), TypeName> -> TypeDefRowId/ExportedTypeToken
        private readonly DoubleHashtable _nestedTypeTokenTable;

        private readonly TypeBase/*?*/[] _moduleTypeDefArray;

        //^ [SpecPublic]
        private readonly LoadState[] _moduleTypeDefLoadState;

        private readonly ExportedTypeAliasBase/*?*/[] _exportedTypeArray;

        //^ [SpecPublic]
        private readonly LoadState[] _exportedTypeLoadState;
        ...
        private void PreLoadTypeDefTableLookup()
        {
            ...
            for (uint i = 1; i <= numberOfTypeDefs; ++i)
            {
                ...
                if (!typeDefRow.IsNested)
                {
                    ...
                    _namespaceTypeTokenTable.Add((uint)namespaceName.UniqueKey, (uint)typeName.UniqueKey, TokenTypeIds.TypeDef | i);
                }
            }
            ...
            for (uint i = 1; i <= numberOfNestedTypes; ++i)
            {
                ...
                _nestedTypeTokenTable.Add(TokenTypeIds.TypeDef | nestedClassRow.EnclosingClass, (uint)typeName.UniqueKey, TokenTypeIds.TypeDef | nestedClassRow.NestedClass);
            }
            uint numberOfExportedTypes = this.PEFileReader.ExportedTypeTable.NumberOfRows;
            for (uint i = 1; i <= numberOfExportedTypes; ++i)
            {
                ...
                {
                    _nestedTypeTokenTable.Add(expTypeRow.Implementation, (uint)typeName.UniqueKey, TokenTypeIds.ExportedType | i);
                }
                else
                {
                    IName namespaceName = this.GetNameFromOffset(expTypeRow.TypeNamespace);
                    _namespaceTypeTokenTable.Add((uint)namespaceName.UniqueKey, (uint)typeName.UniqueKey, TokenTypeIds.ExportedType | i);
                }
            }
        }

        private NamespaceType CreateModuleNamespaceType(
          uint typeDefRowId,
        ...
        private NestedType CreateModuleNestedType(
          uint typeDefRowId,
          TypeDefRow typeDefRow,
        ...
        private ExportedTypeNamespaceAlias CreateExportedNamespaceType(
        ...
        private ExportedTypeNestedAlias CreateExportedNestedType(
          uint exportedTypeRowId,
          ExportedTypeRow exportedTypeRow,
        ...
        //^ requires moduleNamespace.NamespaceNameOffset != 0xFFFFFFFF;
        {
            ...
            uint numberOfTypeDefs = this.PEFileReader.TypeDefTable.NumberOfRows;
            TypeBase/*?*/[] moduleTypeArray = _moduleTypeDefArray;
            for (uint i = 1; i <= numberOfTypeDefs; ++i)
            {
                TypeDefRow typeDefRow = this.PEFileReader.TypeDefTable[i];
                ...
                if (type == null)
                {
                    ...
                    _moduleTypeDefArray[i] = type;
                    _moduleTypeDefLoadState[i] = LoadState.Loaded;
                }
            }
            uint numberOfExportedTypes = this.PEFileReader.ExportedTypeTable.NumberOfRows;
            ExportedTypeAliasBase/*?*/[] exportedTypeArray = _exportedTypeArray;
            for (uint i = 1; i <= numberOfExportedTypes; ++i)
            {
                ...
                if (type == null)
                {
                    ...
                    _exportedTypeArray[i] = type;
                    _exportedTypeLoadState[i] = LoadState.Loaded;
                }
            }
        }
        ...
        {
            if (_moduleTypeDefLoadState[typeDefRowId] == LoadState.Uninitialized)
            {
                Debug.Assert(_moduleTypeDefArray[typeDefRowId] == null);
                ...
                _moduleTypeDefArray[typeDefRowId] = this.CreateModuleNamespaceType(typeDefRowId, typeDefRow, parentNamespace, signatureTypeCode);
                _moduleTypeDefLoadState[typeDefRowId] = LoadState.Loaded;
            }
            TypeBase /*?*/ mt = _moduleTypeDefArray[typeDefRowId];
            //^ assert mt != null;
            return mt;
        }
        ...
        //^ ensures this.ModuleTypeDefLoadState[typeDefRowId] == LoadState.Loaded;
        {
            ...
            Debug.Assert(_moduleTypeDefArray[typeDefRowId] == null);
            _moduleTypeDefLoadState[typeDefRowId] = LoadState.Loading;
            TypeDefRow typeDefRow = this.PEFileReader.TypeDefTable[typeDefRowId];
            ...
            _moduleTypeDefArray[typeDefRowId] = moduleType;
            _moduleTypeDefLoadState[typeDefRowId] = LoadState.Loaded;
            return moduleType;
        }
        ...
        //^ ensures this.ModuleTypeDefLoadState[typeDefRowId] == LoadState.Loaded;
        {
            ...
            {
                CoreTypes dummy = this.CoreTypes; //force core types to be initialized
                TypeBase/*?*/ cachedCopy = _moduleTypeDefArray[typeDefRowId];
                ...
                else
                {
                    ...
                    //type = this.CreateModuleNamespaceType(typeDefRowId, typeDefRow, parentNamespace, ModuleSignatureTypeCode.ValueType);
                }
                _moduleTypeDefArray[typeDefRowId] = type;
                _moduleTypeDefLoadState[typeDefRowId] = LoadState.Loaded;
            }
            TypeBase /*?*/ mt = _moduleTypeDefArray[typeDefRowId];
            return mt;
        ...
        //^ ensures this.ExportedTypeLoadState[exportedTypeRowId] == LoadState.Loaded;
        {
            ...
            if (_exportedTypeLoadState[exportedTypeRowId] == LoadState.Uninitialized)
            {
                Debug.Assert(_exportedTypeArray[exportedTypeRowId] == null);
                _exportedTypeLoadState[exportedTypeRowId] = LoadState.Loading;
                ...
                _exportedTypeArray[exportedTypeRowId] = expType;
                _exportedTypeLoadState[exportedTypeRowId] = LoadState.Loaded;
            }
            ExportedTypeAliasBase/*?*/ et = _exportedTypeArray[exportedTypeRowId];
            return et;
        }
        ...
        {
            uint typeToken = _namespaceTypeTokenTable.Find((uint)namespaceName.UniqueKey, (uint)mangledTypeName.UniqueKey);
        ...
        {
            uint typeToken = _nestedTypeTokenTable.Find(TokenTypeIds.TypeDef | parentType.TypeDefRowId, (uint)typeName.UniqueKey);
            if (typeToken == 0 || ((typeToken & TokenTypeIds.TokenTypeMask) != TokenTypeIds.TypeDef))
            {
                return null;
            }
        ...
        {
            uint exportedTypeToken = _namespaceTypeTokenTable.Find((uint)namespaceName.UniqueKey, (uint)mangledTypeName.UniqueKey);
        ...
        {
            uint exportedTypeToken = _nestedTypeTokenTable.Find(TokenTypeIds.ExportedType | parentType.ExportedTypeRowId, (uint)typeName.UniqueKey);
            if (exportedTypeToken == 0 || ((exportedTypeToken & TokenTypeIds.TokenTypeMask) != TokenTypeIds.ExportedType))
            {
                return null;
            }
        ...
        private readonly GenericParameter[] _moduleGenericParamArray;
        ...
        {
            if (_moduleGenericParamArray[genericParamRowId] == null)
            {
                lock (GlobalLock.LockingObject)
                {
                    if (_moduleGenericParamArray[genericParamRowId] == null)
                    {
                        ...
                        _moduleGenericParamArray[genericParamRowId] = new GenericTypeParameter(this, genericParamRow.Number, genericParamRow.Flags, genericParamName, genericParamRowId, moduleTypeOwner);
                    }
                }
            }
            return _moduleGenericParamArray[genericParamRowId] as GenericTypeParameter;
        }
            ...
            if (_moduleGenericParamArray[genericParamRowId] == null)
            {
                lock (GlobalLock.LockingObject)
                {
                    if (_moduleGenericParamArray[genericParamRowId] == null)
                    {
                        GenericParamRow genericParamRow = this.PEFileReader.GenericParamTable[genericParamRowId];
                        IName genericParamName = this.GetNameFromOffset(genericParamRow.Name);
                        _moduleGenericParamArray[genericParamRowId] = new GenericMethodParameter(this, genericParamRow.Number, genericParamRow.Flags, genericParamName, genericParamRowId, moduleMethodOwner);
                    }
                }
            }
            return _moduleGenericParamArray[genericParamRowId] as GenericMethodParameter;
        }
        ...
        /// </summary>
        private readonly TypeRefReference[] _moduleTypeRefReferenceArray;

        private readonly LoadState[] _moduleTypeRefReferenceLoadState;

        private readonly DoubleHashtable<TypeSpecReference>/*?*/ _moduleTypeSpecHashtable;
        //^ invariant this.PEFileReader.TypeSpecTable.NumberOfRows >= 1 ==> this.ModuleTypeSpecHashtable != null;

        private TypeRefReference CreateTypeRefReference(
          uint typeRefRowId,
        ...
        )
        {
            ...
            Debug.Assert(
              _moduleTypeRefReferenceLoadState[typeRefRowId] == LoadState.Uninitialized
              && _moduleTypeRefReferenceArray[typeRefRowId] == null
            );
            _moduleTypeRefReferenceLoadState[typeRefRowId] = LoadState.Loading;
            ...
            _moduleTypeRefReferenceArray[typeRefRowId] = this.CreateTypeRefReference(typeRefRowId, typeRefRow, null, _assemblyReferenceArray[resolutionScopeRowId], signatureTypeCode);
            _moduleTypeRefReferenceLoadState[typeRefRowId] = LoadState.Loaded;
            return _moduleTypeRefReferenceArray[typeRefRowId];
        }

        private TypeRefReference/*?*/ GetTypeRefReferenceAtRowWorker(
          uint typeRefRowId
        )
        {
        ...
        )
        {
            ...
            if (_moduleTypeRefReferenceLoadState[typeRefRowId] == LoadState.Uninitialized)
            {
                Debug.Assert(_moduleTypeRefReferenceArray[typeRefRowId] == null);
                CoreTypes dummy = this.CoreTypes; //force core types to be initialized
                _moduleTypeRefReferenceLoadState[typeRefRowId] = LoadState.Loading;
                TypeRefRow typeRefRow = this.PEFileReader.TypeRefTable[typeRefRowId];
                ...
                _moduleTypeRefReferenceArray[typeRefRowId] = typeRefReference;
                _moduleTypeRefReferenceLoadState[typeRefRowId] = LoadState.Loaded;
            }
            return _moduleTypeRefReferenceArray[typeRefRowId];
        }
        ...
        //^ requires typeRefRowId >=1;
        {
            ...
            TypeRefReference result = _moduleTypeRefReferenceArray[typeRefRowId];
        ...
        //^ requires typeSpecRowId >= 1 && typeSpecRowId <= this.PEFileReader.TypeSpecTable.NumberOfRows;
        {
            ...
            TypeSpecReference/*?*/ typeSpecReference = _moduleTypeSpecHashtable.Find(ownerId, typeSpecRowId);
            if (typeSpecReference == null)
            {
                lock (GlobalLock.LockingObject)
                {
                    typeSpecReference = _moduleTypeSpecHashtable.Find(ownerId, typeSpecRowId);
                    if (typeSpecReference == null)
                    {
                        typeSpecReference = new TypeSpecReference(this, typeSpecRowId, owningObject);
                    }
                    _moduleTypeSpecHashtable.Add(ownerId, typeSpecRowId, typeSpecReference);
                }
            }
        ...
        private readonly FieldDefinition[] _moduleFieldArray;
        private readonly MethodDefinition[] _moduleMethodArray;
        private readonly EventDefinition[] _moduleEventArray;
        private readonly PropertyDefinition[] _modulePropertyArray;

        private FieldDefinition CreateField(
          uint fieldDefRowId,
          TypeBase parentModuleType
        )
        {
        ...
        private EventDefinition CreateEvent(
          uint eventDefRowId,
          TypeBase parentModuleType
        )
        {
        ...
        private void LoadNestedTypesOfType(
          TypeBase moduleType
        )
        {
            ...
            for (uint i = 1; i <= numberOfNestedClassRows; ++i)
            {
                ...
                if (currType == null)
                {
                    ...
                    _moduleTypeDefArray[nestedClassRow.NestedClass] = currType;
                    _moduleTypeDefLoadState[nestedClassRow.NestedClass] = LoadState.Loaded;
                }
            }
        }
        ...
        )
        {
            ...
            for (uint i = 1; i <= numberOfNestedClassRows; ++i)
            {
                ...
                if (nestedClassRow.EnclosingClass != currentClassRowId)
                    continue;
                INestedTypeDefinition/*?*/ nestedType = _moduleTypeDefArray[nestedClassRow.NestedClass] as INestedTypeDefinition;
                if (nestedType == null)
                {
                    //Err
                    continue;
        ...
        )
        {
            ...
            if (this.PEFileReader.UseFieldPtrTable)
            {
                ...
                for (uint fieldIter = fieldStart; fieldIter < fieldEnd && fieldIter <= numberOfFieldPtrRows; ++fieldIter)
                {
                    ...
                    FieldDefinition moduleField = _moduleFieldArray[fieldRowId];
                    if (moduleField == null)
                    {
                        moduleField = this.CreateField(fieldRowId, moduleType);
                        _moduleFieldArray[fieldRowId] = moduleField;
                    }
                }
            }
            else
            {
                ...
                {
                    FieldDefinition moduleField = _moduleFieldArray[fieldIter];
                    if (moduleField == null)
                    {
                        moduleField = this.CreateField(fieldIter, moduleType);
                        _moduleFieldArray[fieldIter] = moduleField;
                    }
                }
            }
        }
        ...
        )
        {
            ...
            if (this.PEFileReader.UseFieldPtrTable)
            {
                ...
                {
                    uint fieldRowId = this.PEFileReader.FieldPtrTable.GetFieldFor(fieldIter);
                    yield return _moduleFieldArray[fieldRowId];
                }
            }
            else
            {
                ...
                for (uint fieldIter = fieldStart; fieldIter < fieldEnd && fieldIter <= numberOfFieldRows; ++fieldIter)
                {
                    yield return _moduleFieldArray[fieldIter];
                }
            }
        }

        private void LoadMethodsOfType(
          TypeBase moduleType
        )
        {
            uint methodCount;
            ...
            if (this.PEFileReader.UseMethodPtrTable)
            {
                ...
                for (uint methodIter = methodStart; methodIter < methodEnd && methodIter <= numberOfMethodPtrRows; ++methodIter)
                {
                    ...
                    MethodDefinition moduleMethod = _moduleMethodArray[methodRowId];
                    if (moduleMethod == null)
                    {
                        moduleMethod = this.CreateMethod(methodRowId, moduleType);
                        _moduleMethodArray[methodRowId] = moduleMethod;
                    }
                }
            }
            else
            {
                ...
                {
                    MethodDefinition moduleMethod = _moduleMethodArray[methodIter];
                    if (moduleMethod == null)
                    {
                        moduleMethod = this.CreateMethod(methodIter, moduleType);
                        _moduleMethodArray[methodIter] = moduleMethod;
                    }
                }
            }
        }
        ...
        )
        {
            ...
            if (this.PEFileReader.UseMethodPtrTable)
            {
                ...
                {
                    uint methodRowId = this.PEFileReader.MethodPtrTable.GetMethodFor(methodIter);
                    yield return _moduleMethodArray[methodRowId];
                }
            }
            else
            {
                ...
                for (uint methodIter = methodStart; methodIter < methodEnd && methodIter <= numberOfMethodRows; ++methodIter)
                {
                    yield return _moduleMethodArray[methodIter];
                }
            }
        }

        private void LoadEventsOfType(
          TypeBase moduleType
        )
        {
            uint eventCount;
            ...
            if (this.PEFileReader.UseEventPtrTable)
            {
                ...
                for (uint eventIter = eventStart; eventIter < eventEnd && eventIter <= numberOfEventPtrRows; ++eventIter)
                {
                    ...
                    EventDefinition moduleEvent = _moduleEventArray[eventRowId];
                    if (moduleEvent == null)
                    {
                        moduleEvent = this.CreateEvent(eventRowId, moduleType);
                        _moduleEventArray[eventRowId] = moduleEvent;
                    }
                }
            }
            else
            {
                ...
                {
                    EventDefinition moduleEvent = _moduleEventArray[eventIter];
                    if (moduleEvent == null)
                    {
                        moduleEvent = this.CreateEvent(eventIter, moduleType);
                        _moduleEventArray[eventIter] = moduleEvent;
                    }
                }
            }
        }
        ...
        )
        {
            ...
            if (this.PEFileReader.UseEventPtrTable)
            {
                ...
                {
                    uint eventRowId = this.PEFileReader.EventPtrTable.GetEventFor(eventIter);
                    yield return _moduleEventArray[eventRowId];
                }
            }
            else
            {
                ...
                for (uint eventIter = eventStart; eventIter < eventEnd && eventIter <= numberOfEventRows; ++eventIter)
                {
                    yield return _moduleEventArray[eventIter];
                }
            }
        }

        private void LoadPropertiesOfType(
          TypeBase moduleType
        )
        {
            uint propertyCount;
            ...
            if (this.PEFileReader.UsePropertyPtrTable)
            {
                ...
                for (uint propertyIter = propertyStart; propertyIter < propertyEnd && propertyIter <= numberOfPropertyPtrRows; ++propertyIter)
                {
                    ...
                    PropertyDefinition moduleProperty = _modulePropertyArray[propertyRowId];
                    if (moduleProperty == null)
                    {
                        moduleProperty = this.CreateProperty(propertyRowId, moduleType);
                        _modulePropertyArray[propertyRowId] = moduleProperty;
                    }
                }
            }
            else
            {
                ...
                {
                    PropertyDefinition moduleProperty = _modulePropertyArray[propertyIter];
                    if (moduleProperty == null)
                    {
                        moduleProperty = this.CreateProperty(propertyIter, moduleType);
                        _modulePropertyArray[propertyIter] = moduleProperty;
                    }
                }
            }
        }
        ...
        )
        {
            ...
            if (this.PEFileReader.UsePropertyPtrTable)
            {
                ...
                {
                    uint propertyRowId = this.PEFileReader.PropertyPtrTable.GetPropertyFor(propertyIter);
                    yield return _modulePropertyArray[propertyRowId];
                }
            }
            else
            {
                ...
                for (uint propertyIter = propertyStart; propertyIter < propertyEnd && propertyIter <= numberOfPropertyRows; ++propertyIter)
                {
                    yield return _modulePropertyArray[propertyIter];
                }
            }
        }
        ...
        )
        {
            ...
            if (_moduleMethodArray[methodDefRowId] == null)
            {
                lock (GlobalLock.LockingObject)
                {
                    if (_moduleMethodArray[methodDefRowId] == null)
                    {
                        ...
                        uint typeDefRowId = this.PEFileReader.TypeDefTable.FindTypeContainingMethod(methodRow, _moduleMethodArray.Length);
                        if (typeDefRowId == 0 || typeDefRowId > this.PEFileReader.TypeDefTable.NumberOfRows)
                        {
                            // TODO: MD ERRor
                        }
            ...
            MethodDefinition/*?*/ mm = _moduleMethodArray[methodDefRowId];
            return mm;
        }
        ...
        )
        {
            ...
            if (_moduleFieldArray[fieldDefRowId] == null)
            {
                lock (GlobalLock.LockingObject)
                {
                    if (_moduleFieldArray[fieldDefRowId] == null)
                    {
                        ...
                        uint typeDefRowId = this.PEFileReader.TypeDefTable.FindTypeContainingField(fieldRow, _moduleFieldArray.Length);
                        if (typeDefRowId == 0 || typeDefRowId > this.PEFileReader.TypeDefTable.NumberOfRows)
                        {
                            // TODO: MD ERRor
                        }
            ...
            FieldDefinition/*?*/ fd = _moduleFieldArray[fieldDefRowId];
            return fd;
        }
        ...
        private FieldDefinition CreateGlobalField(
          uint fieldDefRowId
        )
        {
            Debug.Assert(fieldDefRowId > 0 && fieldDefRowId <= this.PEFileReader.FieldTable.NumberOfRows);
        ...
        private MethodDefinition CreateGlobalMethod(
          uint methodDefRowId
        )
        {
            Debug.Assert(methodDefRowId > 0 && methodDefRowId <= this.PEFileReader.MethodTable.NumberOfRows);
        ...
        internal void LoadMembersOf_Module_Type()
        {
            ...
            if (this.PEFileReader.UseFieldPtrTable)
            {
                ...
                {
                    uint fieldRowId = this.PEFileReader.FieldPtrTable.GetFieldFor(fieldIter);
                    FieldDefinition globalField = _moduleFieldArray[fieldRowId];
                    if (globalField == null)
                    {
                        globalField = this.CreateGlobalField(fieldRowId);
                        _moduleFieldArray[fieldRowId] = globalField;
                    }
                }
            }
            else
            {
                for (uint fieldIter = fieldStart; fieldIter < fieldEnd; ++fieldIter)
                {
                    FieldDefinition globalField = _moduleFieldArray[fieldIter];
                    if (globalField == null)
                    {
                        globalField = this.CreateGlobalField(fieldIter);
                        _moduleFieldArray[fieldIter] = globalField;
                    }
                }
            }
            ...
            if (this.PEFileReader.UseMethodPtrTable)
            {
                ...
                {
                    uint methodRowId = this.PEFileReader.MethodPtrTable.GetMethodFor(methodIter);
                    MethodDefinition globalMethod = _moduleMethodArray[methodRowId];
                    if (globalMethod == null)
                    {
                        globalMethod = this.CreateGlobalMethod(methodRowId);
                        _moduleMethodArray[methodRowId] = globalMethod;
                    }
                }
            }
            else
            {
                for (uint methodIter = methodStart; methodIter < methodEnd; ++methodIter)
                {
                    MethodDefinition globalMethod = _moduleMethodArray[methodIter];
                    if (globalMethod == null)
                    {
                        globalMethod = this.CreateGlobalMethod(methodIter);
                        _moduleMethodArray[methodIter] = globalMethod;
                    }
                }
            }
        }
        ...
        private readonly MemberReference/*?*/[] _moduleMemberReferenceArray;
        private readonly DoubleHashtable<GenericMethodInstanceReference>/*?*/ _moduleMethodSpecHashtable;
        ...
        )
        {
            ...
            if (_moduleMemberReferenceArray[memberRefRowId] == null)
            {
                ...
                _moduleMemberReferenceArray[memberRefRowId] = retModuleMemberReference;
            }
            MemberReference/*?*/ ret = _moduleMemberReferenceArray[memberRefRowId];
            return ret;
        }
        ...
        )
        {
            ...
            uint ownerId = owningObject.TokenValue;
            GenericMethodInstanceReference/*?*/ methodSpecReference = _moduleMethodSpecHashtable.Find(ownerId, methodSpecRowId);
            if (methodSpecReference == null)
            {
                lock (GlobalLock.LockingObject)
                {
                    methodSpecReference = _moduleMethodSpecHashtable.Find(ownerId, methodSpecRowId);
                    if (methodSpecReference == null)
                    {
                        ...
                        _moduleMethodSpecHashtable.Add(ownerId, methodSpecRowId, methodSpecReference);
                    }
                }
            }
        ...
        private ICustomAttribute/*?*/[] _customAttributeArray;
        private ISecurityAttribute/*?*/[] _declSecurityArray;
        ...
        )
        {
            ...
            if (_customAttributeArray[customAttributeRowId] == null)
            {
                lock (GlobalLock.LockingObject)
                {
                    if (_customAttributeArray[customAttributeRowId] == null)
                    {
                        ...
                        if (customAttribute.Parent == owningObject.TokenValue || (customAttribute.Parent == 1 && owningObject.TokenValue == TokenTypeIds.Assembly + 1))
                        {
                            ...
                            {
                                //  TODO: MDError
                                _customAttributeArray[customAttributeRowId] = Dummy.CustomAttribute;
                                return Dummy.CustomAttribute;
                            }
                            if (customAttribute.Value == 0)
                            {
                                _customAttributeArray[customAttributeRowId] =
                            ...
                            else
                            {
                                ...
                                _customAttributeArray[customAttributeRowId] = customAttrDecoder.CustomAttribute;
                            }
                        }
                        else
                        {
            ...
            ICustomAttribute/*?*/ ret = _customAttributeArray[customAttributeRowId];
            //^ assert ret != null;
            return ret;
        }
        ...
        {
            if (securityAttributeRowId >= _declSecurityArray.Length)
            {
                //  MD Error
                return Dummy.SecurityAttribute;
            }
            if (_declSecurityArray[securityAttributeRowId] == null)
            {
                lock (GlobalLock.LockingObject)
                {
                    if (_declSecurityArray[securityAttributeRowId] == null)
                    {
                        ...
                        {
                            _declSecurityArray[securityAttributeRowId] = new SecurityAttribute(this, securityAttributeRowId, (SecurityAction)declSecurity.ActionFlags);
                        }
                        else
                        {
                            //  MD Error
                            _declSecurityArray[securityAttributeRowId] = Dummy.SecurityAttribute;
                        }
                    }
                }
            }
            ISecurityAttribute/*?*/ ret = _declSecurityArray[securityAttributeRowId];
            //^ assert ret != null;
            return ret;
        }
    ...
    internal sealed class MethodDefSignatureConverter : SignatureConverter
    {
        ...
        private readonly ParamInfo[] _paramInfoArray;
          ...
          : base(peFileToObjectModel, signatureMemoryReader, moduleMethod)
        {
            ...
            _paramInfoArray = paramInfoArray;
            if (paramCount > 0)
            {
        ...
        {
            for (int i = 0; i < _paramInfoArray.Length; ++i)
            {
                if (paramSequence == _paramInfoArray[i].ParamSequence)
                    return _paramInfoArray[i];
            }
            return null;
        }
    }
