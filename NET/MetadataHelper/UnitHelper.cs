...
namespace Microsoft.Cci
{
    ...
    public sealed class UnitSet : IUnitSet
    {
        ...
        {
            bool result = IteratorHelper.EnumerableContains(_units, unit);
                ...
                if (_unitSetNamespaceRoot == null)
                {
                    lock (GlobalLock.LockingObject)
                    {
                        if (_unitSetNamespaceRoot == null)
                        {
                            ...
                            _unitSetNamespaceRoot = new RootUnitSetNamespace(rootName, this);
                            _unitSetNamespaceRoot.Locations.GetEnumerator().MoveNext();
                        }
                    }
                }
                //^ assume false;
                return _unitSetNamespaceRoot;
            }
        }

        private IRootUnitSetNamespace/*?*/ _unitSetNamespaceRoot;
    ...
    public abstract class UnitSetNamespace : AggregatedNamespace<INamespaceDefinition, INamespaceMember>, IUnitSetNamespace
    {
        ...
        {
            _unitSet = unitSet;
            _unitNamespaces = nestedUnitNamespaces;
        }
        ...
        protected override INamespaceMember GetAggregatedMember(INamespaceMember member)
        {
            ...
            if (!_nestedUnitNamespaceToNestedUnitSetNamespaceMap.TryGetValue(nestedUnitNamespace.Name.UniqueKey, out result))
            {
                result = new NestedUnitSetNamespace(this, member.Name, _unitSet, new List<IUnitNamespace>());
                _nestedUnitNamespaceToNestedUnitSetNamespaceMap.Add(nestedUnitNamespace.Name.UniqueKey, result);
            }
            //^ assume result != null && result.unitNamespaces != null;
            result._unitNamespaces.Add(nestedUnitNamespace);
            //TODO: thread safety
            return result;
        }

        private readonly Dictionary<int, NestedUnitSetNamespace> _nestedUnitNamespaceToNestedUnitSetNamespaceMap = new Dictionary<int, NestedUnitSetNamespace>();
        ...
        protected List<IUnitNamespace> UnitNamespaces
        {
            get
            {
                if (_unitNamespaces == null)
                {
                    lock (GlobalLock.LockingObject)
                    {
                        if (_unitNamespaces == null)
                        {
                            List<IUnitNamespace> unitNamespaces = _unitNamespaces = new List<IUnitNamespace>();
                            foreach (IUnit unit in _unitSet.Units)
                            {
                ...
                return _unitNamespaces;
            }
        }

        private List<IUnitNamespace>/*?*/ _unitNamespaces;
        ...
        private readonly IUnitSet _unitSet;
    }
    ...
    public sealed class NestedUnitSetNamespace : UnitSetNamespace, INamespaceMember, INestedUnitSetNamespace
    {
        ...
        private readonly UnitSetNamespace _containingNamespace;
        ...
        #endregion IContainerMember<INamespace> Members
        ...
        #endregion INestedUnitSetNamespace Members
    }
}