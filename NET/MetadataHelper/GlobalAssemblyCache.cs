...
namespace Microsoft.Cci
{
    ...
    {
        private static bool s_fusionLoaded;
    ...
    {
        private IAssemblyName _assemblyName;
        ...
        internal string StrongName
        {
            ...
            {
                uint usize = 0;
                _assemblyName.GetDisplayName(null, ref usize, (uint)AssemblyNameDisplayFlags.ALL);
                int size = (int)usize;
                if (size <= 0) return "";
                StringBuilder strongName = new StringBuilder(size);
                _assemblyName.GetDisplayName(strongName, ref usize, (uint)AssemblyNameDisplayFlags.ALL);
        ...
        {
            uint size = 0;
            _assemblyName.GetProperty(assemblyNameProperty, IntPtr.Zero, ref size);
            if (size == 0 || size > Int16.MaxValue) return String.Empty;
            IntPtr ptr = Marshal.AllocHGlobal((int)size);
            _assemblyName.GetProperty(assemblyNameProperty, ptr, ref size);
