...
namespace AIMS.Libraries.Forms.Docking
{
    ...
    internal sealed class DragHandler : DragHandlerBase
    {
        ...
        private DockOutlineBase _dockOutline;
        internal DockOutlineBase DockOutline
        {
            get { return _dockOutline; }
        }

        private SplitterOutline _splitterOutline;
        private SplitterOutline SplitterOutline
        {
            get { return _splitterOutline; }
        }

        private DockIndicator _dockIndicator;
        private DockIndicator DockIndicator
        {
            get { return _dockIndicator; }
        }
        ...
        private bool InitDrag(Control c, DragSource source)
        {
            ...
            {
                _dockOutline = new DockOutline();
                _dockIndicator = new DockIndicator(this);
                ...
                _splitterOutline = new SplitterOutline();
            else
                return false;
        ...
        private void Content_BeginDrag(IDockableWindow content)
        {
            ...
            _mouseOffset.X = pt.X - StartMousePosition.X;
            _mouseOffset.Y = pt.Y - StartMousePosition.Y;
        }
        ...
        {
            _splitterLocation = DragControl.PointToScreen(ptSplitter);
            Point ptMouse = StartMousePosition;

            _mouseOffset.X = _splitterLocation.X - ptMouse.X;
            _mouseOffset.Y = _splitterLocation.Y - ptMouse.Y;
        ...
        private void AutoHideWindowSplitter_OnEndDrag(bool abort)
        {
            ...
            DockContainer dockPanel = autoHideWindow.DockPanel;
            DockState state = autoHideWindow.DockState;

            Point pt = _splitterLocation;
            Rectangle rect = GetWindowSplitterDragRectangle();
            Rectangle rectDockArea = dockPanel.DockArea;
            IDockableWindow content = dockPanel.ActiveAutoHideContent;
        ...
        {
            _splitterLocation = DragControl.Parent.PointToScreen(ptSplitter);
            Point ptMouse = StartMousePosition;
        ...
        private Rectangle GetSplitterDragRectangle(Rectangle rectLimit, bool bVerticalSplitter)
        {
            ...
            {
                rect.X = ptMouse.X + _mouseOffset.X;
                rect.Y = pt.Y;
            ...
            {
                rect.X = pt.X;
                rect.Y = ptMouse.Y + _mouseOffset.Y;
        ...
        {
            _mouseOffset.X = DragControl.Bounds.X - StartMousePosition.X;
            _mouseOffset.Y = DragControl.Bounds.Y - StartMousePosition.Y;
        }
            ...
            else if (DockOutline.DockTo is DockPane)
            {
                ...
                if (DockOutline.Dock == DockStyle.Fill)
                {
                    for (int i = floatWindow.DockList.Count - 1; i >= 0; i--)
                    {
                        ...
                        {
                            IDockableWindow c = pane.Contents[j];
                            c.DockHandler.Pane = paneTo;
                            if (DockOutline.ContentIndex != -1)
                                paneTo.SetContentIndex(c, DockOutline.ContentIndex);
                            c.DockHandler.Activate();
        ...
        public bool IsDockStateValid(DockState dockState)
        {
            if (Source == DragSource.FloatWindow)
                ...
                return ((IDockableWindow)DragControl).DockHandler.IsDockStateValid(dockState);
            else
                return false;
        }
        ...
        private Rectangle GetFloatWindowBounds()
        {
            ...
            if (Source == DragSource.Content)
            {
                IDockableWindow content = (IDockableWindow)DragControl;
                Size size = FloatWindow.DefaultWindowSize;
                Point location;
                if (content.DockHandler.DockState == DockState.Document)
                    location = new Point(ptMouse.X + _mouseOffset.X, ptMouse.Y + _mouseOffset.Y);
                else
                    location = new Point(ptMouse.X + _mouseOffset.X, ptMouse.Y + _mouseOffset.Y - size.Height);
            ...
            {
                DockPane pane = (DockPane)DragControl;
                Point location = new Point(ptMouse.X + _mouseOffset.X, ptMouse.Y + _mouseOffset.Y);
                Size size;
