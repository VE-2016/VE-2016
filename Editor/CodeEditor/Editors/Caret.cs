...
namespace AIMS.Libraries.CodeEditor.WinForms
{
    ...
    public sealed class Caret
    {
        ...
        public void CropPosition()
        {
            ...
            if (this.Position.Y >= _control.Document.Count)
                this.Position.Y = _control.Document.Count - 1;
            ...
            if (this.Position.X > xtr.Text.Length && !_control.VirtualWhitespace)
        ...
        public void MoveRight(bool Select)
        {
            ...
            else
            {
                ...
                if (this.Position.X > xtr.Text.Length && !_control.VirtualWhitespace)
                {
                    if (this.Position.Y < _control.Document.Count - 1)
                    {
        ...
        {
            this.Position.X = _control.Document[_control.Document.Count - 1].Text.Length;
            ;
            this.Position.Y = _control.Document.Count - 1;
            RememberXPos();
            CaretMoved(Select);
        ...
        {
            get { return _control.Document.GetWordFromPos(this.Position); }
        }
        ...
        {
            return _control.Document.GetSegmentFromPos(this.Position);
        }
        ...
        {
            get { return _control.Document[this.Position.Y]; }
        }
        ...
        public int Offset
        {
            get
            {
                if (this.Position.Y >= _control.Document.Lines.Length)
                    return 0;
                TextRange tr = new TextRange(0, 0, this.Position.X, this.Position.Y);
                int l = _control.Document.GetRange(tr).Length;
                int tl = _control.Document.Text.Length;
                return Math.Min(tl, l);
            }
        ...
        public TextPoint LogicalPosition
        {
            get
            {
                ...
                foreach (char c in TotStr.ToCharArray(0, this.Position.X))
                {
                    if (c == '\t')
                    {
                        x += _control.TabSize - (x % _control.TabSize);
                    }
                    else
                    {
                        x++;
