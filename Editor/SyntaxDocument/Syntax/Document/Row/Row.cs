...
namespace AIMS.Libraries.CodeEditor.Syntax
{
    ...
    public sealed class Row : IEnumerable
    {
        ...
        private bool _mBookmarked = false; //is this line bookmarked?
        private bool _mBreakpoint = false; //Does this line have a breakpoint?
                                          /// <summary>
                                          /// For public use only
                                          /// </summary>
        ...
        {
            string s = "";
            foreach (Word w in mWords.ToArray())
            {
                s = s + w.Text;
            }
            _mText = s;
        ...
        public Word GetCaretWord(int X)
        {
            foreach (Word w in this)
            {
                if (w.Type == WordType.xtWord)
                    if (w.Column <= X)
                        if (w.Text.Length + w.Column >= X)
                            return w;
            }
            return null;
        }
        ...
        public string PrevNonWs(AIMS.Libraries.CodeEditor.Syntax.Word w)
        {
            string ww = "";
        ...
        public bool HasDots(AIMS.Libraries.CodeEditor.Syntax.Word w)
        {
            var v = mWords;
        ...
        public string[] Words(string word)
        {
            ...
            foreach (Word w in mWords)
            {
                if (w.Text == word)
                {
                    ...
                    if (i - 2 < mWords.Count)
                    {
                        if (mWords.Count <= 1 || i < 1)
                            words[0] = "";
                        else if (mWords[i - 1].Text != ".")
                            words[0] = mWords[i - 1].Text;
                        else if (i - 2 >= 0)
                            words[0] = mWords[i - 1].Text;
                        // words[0] = mWords[i - 2].Text;
                    }

                    if (i + 1 >= mWords.Count)
                        words[2] = "";
                    else if (i + 2 < mWords.Count)
                    {
                        if (mWords[i + 1].Text == ".")
                            words[2] = mWords[i + 2].Text;
                    }
                    else
                        words[2] = mWords[i + 1].Text;
