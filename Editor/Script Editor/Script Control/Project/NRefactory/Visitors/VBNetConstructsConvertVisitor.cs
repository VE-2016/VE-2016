...
using Attribute = AIMS.Libraries.Scripting.NRefactory.Ast.Attribute;
...
namespace AIMS.Libraries.Scripting.NRefactory.Visitors
{
    ...
    public class VBNetConstructsConvertVisitor : AbstractAstTransformer
    {
        ...
        private Dictionary<string, string> _usings;
        private List<UsingDeclaration> _addedUsings;
        private TypeDeclaration _currentTypeDeclaration;

        public override object VisitCompilationUnit(CompilationUnit compilationUnit, object data)
        {
            _usings = new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase);
            _addedUsings = new List<UsingDeclaration>();
            ...
            foreach (UsingDeclaration decl in _addedUsings)
            {
        ...
        {
            if (_usings != null && !@using.IsAlias)
            {
                _usings[@using.Name] = @using.Name;
            }
        ...
        public override object VisitTypeDeclaration(TypeDeclaration typeDeclaration, object data)
        {
            ...
            if (_currentTypeDeclaration != null && (typeDeclaration.Modifier & Modifiers.Visibility) == 0)
        ...
        {
            // fix default visibility of inner classes
            if (_currentTypeDeclaration != null && (delegateDeclaration.Modifier & Modifiers.Visibility) == 0)
        ...
        {
            if (_currentTypeDeclaration == null) return false;
            return _currentTypeDeclaration.Type == c;
        }
        ...
        {
            if (_usings != null && !_usings.ContainsKey("System.Runtime.InteropServices"))
            {
                UsingDeclaration @using = new UsingDeclaration("System.Runtime.InteropServices");
                _addedUsings.Add(@using);
        ...
        private class ReturnStatementForFunctionAssignment : AbstractAstTransformer
        {
            ...
            {
                if (identifierExpression.Identifier.Equals(_functionName, StringComparison.InvariantCultureIgnoreCase))
                {
        ...
        private class RenameIdentifierVisitor : AbstractAstVisitor
        {
            ...
            public override object VisitIdentifierExpression(IdentifierExpression identifierExpression, object data)
            {
                if (string.Equals(identifierExpression.Identifier, _from, StringComparison.InvariantCultureIgnoreCase))
                {
                    identifierExpression.Identifier = _to;
                }
                return base.VisitIdentifierExpression(identifierExpression, data);
            }
        }

        private static volatile Dictionary<string, Expression> s_constantTable;
        private static volatile Dictionary<string, Expression> s_methodTable;
        ...
        public override object VisitIdentifierExpression(IdentifierExpression identifierExpression, object data)
        {
            ...
            {
                s_constantTable = CreateDictionary("Constants");
            }
            Expression expr;
            if (s_constantTable.TryGetValue(identifierExpression.Identifier, out expr))
            {
        ...
        public override object VisitInvocationExpression(InvocationExpression invocationExpression, object data)
        {
            ...
            if (ident != null)
            {
                ...
                if (s_methodTable == null)
                {
                    s_methodTable = CreateDictionary("Conversion", "FileSystem", "Financial", "Information",
                                                   "Interaction", "Strings", "VBMath");
                }
                Expression expr;
                if (s_methodTable.TryGetValue(ident.Identifier, out expr))
                {
