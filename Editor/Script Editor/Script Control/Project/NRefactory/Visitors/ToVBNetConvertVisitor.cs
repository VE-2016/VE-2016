...
namespace AIMS.Libraries.Scripting.NRefactory.Visitors
{
    ...
    public class ToVBNetConvertVisitor : AbstractAstTransformer
    {
        ...
        private List<INode> _nodesToMoveToCompilationUnit = new List<INode>();

        public override object VisitCompilationUnit(CompilationUnit compilationUnit, object data)
        {
            base.VisitCompilationUnit(compilationUnit, data);
            for (int i = 0; i < _nodesToMoveToCompilationUnit.Count; i++)
            {
                compilationUnit.Children.Insert(i, _nodesToMoveToCompilationUnit[i]);
                _nodesToMoveToCompilationUnit[i].Parent = compilationUnit;
            }
            return null;
        }
        ...
        public override object VisitUsingDeclaration(UsingDeclaration usingDeclaration, object data)
        {
            ...
            {
                _nodesToMoveToCompilationUnit.Add(usingDeclaration);
                RemoveCurrentNode();
            }
            return null;
        }
        ...
        public override object VisitTypeDeclaration(TypeDeclaration typeDeclaration, object data)
        {
            ...
            if (_currentType != null && (typeDeclaration.Modifier & Modifiers.Visibility) == 0)
                typeDeclaration.Modifier |= Modifiers.Private;

            TypeDeclaration outerType = _currentType;
        ...
        {
            // fix default inner type visibility
            if (_currentType != null && (delegateDeclaration.Modifier & Modifiers.Visibility) == 0)
        ...
        private string GetAnonymousMethodName()
        {
            for (int i = 1; ; i++)
            {
                ...
                {
                    foreach (object c in _currentType.Children)
                    {
        ...
        public override object VisitAnonymousMethodExpression(AnonymousMethodExpression anonymousMethodExpression, object data)
        {
            ...
            if (_currentType != null)
            {
                _currentType.Children.Add(method);
            }
        ...
        public override object VisitAssignmentExpression(AssignmentExpression assignmentExpression, object data)
        {
                ...
                || assignmentExpression.Op == AssignmentOperatorType.Subtract)
            {
                ...
                if (methodName != null && _currentType != null)
                {
                    foreach (object c in _currentType.Children)
                    {
                        MethodDeclaration method = c as MethodDeclaration;
        ...
        {
            if (_currentType == null) return false;
            return _currentType.Type == c;
        }
