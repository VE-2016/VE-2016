...
namespace AIMS.Libraries.Scripting.NRefactory.Parser.VB
{
    internal sealed class Lexer : AbstractLexer
    {
        ...
        public override Token NextToken()
        {
            ...
            if (curToken.kind == Tokens.EOF && !(lastToken.kind == Tokens.EOL))
            { // be sure that before EOF there is an EOL token
        ...
        private Token ReadDigit(char ch, int x)
        {
            ...
            if (ReaderPeek() != -1 && PeekUpperChar() == 'E')
            { // read exponent
                isdouble = true;
                digit += (char)ReaderRead();
