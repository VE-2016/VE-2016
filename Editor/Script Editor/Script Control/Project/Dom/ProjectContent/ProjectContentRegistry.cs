...
namespace AIMS.Libraries.Scripting.Dom
{
    ...
    public class ProjectContentRegistry : IDisposable
    {
        ...
        private Dictionary<string, IProjectContent> _contents = new Dictionary<string, IProjectContent>(StringComparer.InvariantCultureIgnoreCase);
        ...
        public virtual void Dispose()
        {
            ...
            lock (_contents)
            {
                list = new List<IProjectContent>(_contents.Values);
                _contents.Clear();
            }
        ...
        private ReflectionProjectContent _mscorlibContent;
        ...
        public virtual IProjectContent Mscorlib
        {
            get
            {
                if (_mscorlibContent != null) return _mscorlibContent;
                lock (_contents)
                {
                    if (_contents.ContainsKey("mscorlib"))
                    {
                        _mscorlibContent = (ReflectionProjectContent)_contents["mscorlib"];
                        return _contents["mscorlib"];
                    }
                    int time = LoggingService.IsDebugEnabled ? Environment.TickCount : 0;
                    LoggingService.Debug("Loading PC for mscorlib...");
                    if (persistence != null)
                    {
                        _mscorlibContent = persistence.LoadProjectContentByAssemblyName(MscorlibAssembly.FullName);
                        if (_mscorlibContent != null)
                        {
                            if (time != 0)
                            {
                    ...
                    if (_mscorlibContent == null)
                    {
                        ...
                        _mscorlibContent = new ReflectionProjectContent(MscorlibAssembly, this);
                        if (time != 0)
                        {
                        ...
                        if (persistence != null)
                        {
                            persistence.SaveProjectContent(_mscorlibContent);
                            LoggingService.Debug("Saved mscorlib to cache");
                        }
                    }
                    _contents["mscorlib"] = _mscorlibContent;
                    _contents[_mscorlibContent.AssemblyFullName] = _mscorlibContent;
                    _contents[_mscorlibContent.AssemblyLocation] = _mscorlibContent;
                    lock (redirectedAssemblyNames)
                    {
                        redirectedAssemblyNames.Add("mscorlib", _mscorlibContent.AssemblyFullName);
                    }
                    return _mscorlibContent;
                }
            }
        }
        ...
        {
            lock (_contents)
            { // we need to return a copy because we have to lock
                return new List<IProjectContent>(_contents.Values);
            }
        }
        ...
        public virtual IProjectContent GetExistingProjectContent(string itemInclude, string itemFileName)
        {
            ...
            lock (_contents)
            {
                if (_contents.ContainsKey(itemFileName))
                {
                    return _contents[itemFileName];
                }
                if (_contents.ContainsKey(itemInclude))
                {
                    return _contents[itemInclude];
                }
            }
            return null;
        }
        ...
        protected virtual IProjectContent LoadProjectContent(string itemInclude, string itemFileName)
        {
            ...
            {
                if (pc == null)
                    pc = ReflectionLoadProjectContent(itemFileName, itemInclude);
