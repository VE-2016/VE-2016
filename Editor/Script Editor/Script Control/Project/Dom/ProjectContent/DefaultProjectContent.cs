...
namespace AIMS.Libraries.Scripting.Dom
{
    ...
    {
        private readonly List<IProjectContent> _referencedContents = new List<IProjectContent>();
        ...
        private List<Dictionary<string, IClass>> _classLists = new List<Dictionary<string, IClass>>();
        private List<Dictionary<string, NamespaceStruct>> _namespaces = new List<Dictionary<string, NamespaceStruct>>();

        private XmlDoc _xmlDoc = new XmlDoc();
        private IUsing _defaultImports;
        ...
        public List<Dictionary<string, IClass>> ClassLists
        {
            get
            {
                ...
                {
                    _classLists.Add(new Dictionary<string, IClass>(_language.NameComparer));
                }
                return _classLists;
            }
        }
        ...
        protected List<Dictionary<string, NamespaceStruct>> Namespaces
        {
            get
            {
                ...
                {
                    _namespaces.Add(new Dictionary<string, NamespaceStruct>(_language.NameComparer));
                }
                return _namespaces;
            }
        }
        ...
        {
            for (int i = 0; i < _classLists.Count; ++i)
            {
                if (_classLists[i].Comparer == language.NameComparer)
                    return _classLists[i];
            }
            Dictionary<string, IClass> d;
            if (_classLists.Count > 0)
            {
                Dictionary<string, IClass> oldList = _classLists[0];
        ...
        protected Dictionary<string, NamespaceStruct> GetNamespaces(LanguageProperties language)
        {
            ...
            {
                if (_namespaces[i].Comparer == language.NameComparer)
                    return _namespaces[i];
            }
            Dictionary<string, NamespaceStruct> d;
            if (_namespaces.Count > 0)
            {
                Dictionary<string, NamespaceStruct> oldList = _namespaces[0];
        ...
        public virtual SystemTypes SystemTypes
        {
            get
            {
                ...
                {
                    _systemTypes = new SystemTypes(this);
                }
                return _systemTypes;
            }
        }
        ...
        private LanguageProperties _language = LanguageProperties.CSharp;
        ...
        {
            string desc = _xmlDoc.GetDocumentation(memberTag);
            if (desc != null)
            {
                return desc;
            }
            ...
            {
                foreach (IProjectContent referencedContent in _referencedContents)
                {
        ...
        private class GenericClassContainer : DefaultClass
        {
            ...
            private IClass[] _realClasses = new IClass[4];

            public IEnumerable<IClass> RealClasses
            {
                get
                {
                    foreach (IClass c in _realClasses)
                    {
            ...
            {
                if (_realClasses.Length > typeParameterCount)
                    return _realClasses[typeParameterCount];
                else
                    return null;
            }
            ...
            {
                IClass c;
                for (int i = typeParameterCount; i < _realClasses.Length; i++)
                {
                    c = Get(i);
                    if (c != null) return c;
                }
            ...
            public void Set(IClass c)
            {
                ...
                if (_realClasses.Length <= typeParameterCount)
                {
                    ...
                    _realClasses.CopyTo(newArray, 0);
                    _realClasses = newArray;
                }
                _realClasses[typeParameterCount] = c;
            }

            public void Remove(int typeParameterCount)
            {
                if (_realClasses.Length > typeParameterCount)
                    _realClasses[typeParameterCount] = null;
            }
        }
        ...
        protected void AddClassToNamespaceListInternal(IClass addClass)
        {
            ...
            IClass existingClass = GetClassInternal(fullyQualifiedName, addClass.TypeParameters.Count, _language);
            if (existingClass != null && existingClass.TypeParameters.Count == addClass.TypeParameters.Count)
            {
                ...
                else if (addClass.IsPartial || _language.ImplicitPartialClasses)
                {
            ...
            if (GetClasses(_language).TryGetValue(fullyQualifiedName, out oldDictionaryClass))
            {
            ...
            List<IClass> classList = GetNamespaces(_language)[nSpace].Classes;
        ...
        {
            Dictionary<string, NamespaceStruct> dict = GetNamespaces(_language);
            ...
            foreach (Dictionary<string, NamespaceStruct> otherDict in _namespaces)
            {
        ...
        private void RemoveEmptyNamespace(string nSpace)
        {
            ...
            foreach (Dictionary<string, NamespaceStruct> anyDict in _namespaces)
            {
        ...
        private void RemoveClass(IClass @class)
        {
            ...
            if (@class.IsPartial)
            {
                ...
                CompoundClass compound = GetClassInternal(fullyQualifiedName, typeParameterCount, _language) as CompoundClass;
            ...
            if (!GetClasses(_language).TryGetValue(fullyQualifiedName, out classInDictionary))
            {
                return;
            }
            ...
            for (int i = 0; i < classList.Count; i++)
            {
                if (_language.NameComparer.Equals(classList[i].FullyQualifiedName, fullyQualifiedName))
                {
        ...
        {
            return GetClass(typeName, typeParameterCount, _language, true);
        }
        ...
        public ArrayList GetNamespaceContents(string nameSpace)
        {
            ...
            AddNamespaceContents(namespaceList, nameSpace, _language, true);
        ...
        {
            return NamespaceExists(name, _language, true);
        }
        ...
        public string SearchNamespace(string name, IClass curType, ICompilationUnit unit, int caretLine, int caretColumn)
        {
            ...
            {
                string nameSpace = _defaultImports.SearchNamespace(name);
                if (nameSpace != null)
                {
                    return nameSpace;
                }
        ...
        public SearchTypeResult SearchType(SearchTypeRequest request)
        {
                ...
                if (name.IndexOf('.') < 0)
                {
                    ...
                    foreach (IClass baseClass in request.CurrentType.ClassInheritanceTree)
                    {
                        if (baseClass.ClassType == ClassType.Class)
                        {
                            ...
                            {
                                if (_language.NameComparer.Equals(innerClass.Name, name))
            ...
            if (_defaultImports != null)
            {
                foreach (IReturnType r in _defaultImports.SearchType(name, request.TypeParameterCount))
                {
                    if (r.TypeParameterCount == request.TypeParameterCount)
                    {
                        return new SearchTypeResult(r, _defaultImports);
                    }
                    else
                    {
                        fallbackResult = new SearchTypeResult(r, _defaultImports);
                    }
                }
            }
        ...
        {
            _systemTypes = null; // re-create system types
            SearchClassReturnType.ClearCache();
