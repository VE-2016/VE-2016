...
namespace AIMS.Libraries.Scripting.Dom
{
    ...
    public sealed class ConstructedReturnType : ProxyReturnType
    {
        ...
        private IList<IReturnType> _typeArguments;

        private IReturnType _baseType;
        ...
        {
            int code = _baseType.GetHashCode();
            foreach (IReturnType t in _typeArguments)
            {
                if (t != null)
                {
                    code ^= t.GetHashCode();
        ...
        public override string DotNetName
        {
            get
            {
                string baseName = _baseType.DotNetName;
                ...
                {
                    if (i > 0) b.Append(',');
                    if (_typeArguments[i] != null)
                    {
                        b.Append(_typeArguments[i].DotNetName);
                    }
                }
                b.Append('}');
                return b.ToString();
        ...
        {
            return TranslateType(input, _typeArguments, false);
        }

        public override List<IMethod> GetMethods()
        {
            List<IMethod> l = _baseType.GetMethods();
            for (int i = 0; i < l.Count; ++i)
            {
                if (CheckReturnType(l[i].ReturnType) || CheckParameters(l[i].Parameters))
                {
                    l[i] = (IMethod)l[i].Clone();
                    if (l[i].DeclaringType == _baseType.GetUnderlyingClass())
                    {
        ...
        {
            List<IProperty> l = _baseType.GetProperties();
            for (int i = 0; i < l.Count; ++i)
            {
                if (CheckReturnType(l[i].ReturnType) || CheckParameters(l[i].Parameters))
                {
        ...
        {
            List<IField> l = _baseType.GetFields();
            for (int i = 0; i < l.Count; ++i)
            {
        ...
        {
            List<IEvent> l = _baseType.GetEvents();
            for (int i = 0; i < l.Count; ++i)
            {
                if (CheckReturnType(l[i].ReturnType))
                {
