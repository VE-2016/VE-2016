    ...
    public class NRefactoryRefactoringProvider : RefactoringProvider
    {
        ...
        public override bool IsEnabledForFile(string fileName)
        {
            ...
            if (extension.Equals(".cs", StringComparison.InvariantCultureIgnoreCase))
                return _language == NR.SupportedLanguage.CSharp;
            else if (extension.Equals(".vb", StringComparison.InvariantCultureIgnoreCase))
                return _language == NR.SupportedLanguage.VBNet;
            else
                return false;
        }
        ...
        private NR.IParser ParseFile(string fileContent)
        {
            NR.IParser parser = NR.ParserFactory.CreateParser(_language, new StringReader(fileContent));
            parser.Parse();
        ...
        public override string CreateNewFileLikeExisting(string existingFileContent, string codeForNewType)
        {
            ...
            IOutputAstVisitor outputVisitor = (_language == NR.SupportedLanguage.CSharp) ? new CSharpOutputVisitor() : (IOutputAstVisitor)new VBNetOutputVisitor();
            using (SpecialNodesInserter.Install(comments, outputVisitor))
            {
                parser.CompilationUnit.AcceptVisitor(outputVisitor, null);
            }
            string expectedText;
            if (_language == NR.SupportedLanguage.CSharp)
        ...
        {
            NR.Parser.ILexer lexer = NR.ParserFactory.CreateLexer(_language, new StringReader(fileContent));
            ...
            bool csharp = _language == NR.SupportedLanguage.CSharp;
