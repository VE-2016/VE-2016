...
namespace AIMS.Libraries.Scripting.Dom.NRefactoryResolver
{
    ...
    {
        private NRefactoryResolver _resolver;
        ...
        public override object VisitPrimitiveExpression(PrimitiveExpression primitiveExpression, object data)
        {
            ...
            {
                return _resolver.ProjectContent.SystemTypes.CreatePrimitive(primitiveExpression.Value.GetType());
            }
        }
        ...
        public override object VisitBinaryOperatorExpression(BinaryOperatorExpression binaryOperatorExpression, object data)
        {
            switch (binaryOperatorExpression.Op)
            {
                ...
                case BinaryOperatorType.DivideInteger:
                    return _resolver.ProjectContent.SystemTypes.Int32;
                case BinaryOperatorType.Concat:
                    return _resolver.ProjectContent.SystemTypes.String;
                ...
                case BinaryOperatorType.GreaterThanOrEqual:
                    return _resolver.ProjectContent.SystemTypes.Boolean;
                default:
                    return MemberLookupHelper.GetCommonType(_resolver.ProjectContent,
        ...
        public IMethodOrProperty GetMethod(InvocationExpression invocationExpression)
        {
            ...
            if (invocationExpression.TargetObject is FieldReferenceExpression)
            {
                ...
                List<IMethod> methods = _resolver.SearchMethod(type, field.FieldName);
                if (methods.Count == 0 && _resolver.Language == SupportedLanguage.VBNet)
            ...
            else if (invocationExpression.TargetObject is IdentifierExpression)
            {
                ...
                if (_resolver.CallingClass == null)
                {
                    return null;
                }
                List<IMethod> methods = _resolver.SearchMethod(id);
                if (methods.Count == 0 && _resolver.Language == SupportedLanguage.VBNet)
                    return GetVisualBasicIndexer(invocationExpression);
                return FindOverload(methods, typeParameters, invocationExpression.Arguments, null);
            }
        ...
        public override object VisitFieldReferenceExpression(FieldReferenceExpression fieldReferenceExpression, object data)
        {
            ...
            if (returnType != null)
            {
                if (returnType is NamespaceReturnType)
                {
                    ...
                    if (_resolver.ProjectContent.NamespaceExists(combinedName))
                    {
                        return new NamespaceReturnType(combinedName);
                    }
                    IClass c = _resolver.GetClass(combinedName);
                    if (c != null)
                    {
                        return c.DefaultReturnType;
                    }
                    if (_resolver.LanguageProperties.ImportModules)
                    {
                        // go through the members of the modules
                        foreach (object o in _resolver.ProjectContent.GetNamespaceContents(name))
                        {
                            IMember member = o as IMember;
                            if (member != null && _resolver.IsSameName(member.Name, fieldReferenceExpression.FieldName))
                                return member.ReturnType;
                        }
                    }
                    return null;
                }
                return _resolver.SearchMember(returnType, fieldReferenceExpression.FieldName);
            }
            return null;
        }
        ...
        public override object VisitIdentifierExpression(IdentifierExpression identifierExpression, object data)
        {
            ...
            IClass c = _resolver.SearchClass(identifierExpression.Identifier);
            if (c != null)
            {
                return c.DefaultReturnType;
            }
            return _resolver.DynamicLookup(identifierExpression.Identifier);
        }
        ...
        {
            return _resolver.ProjectContent.SystemTypes.Type;
        }
        ...
        public override object VisitClassReferenceExpression(ClassReferenceExpression classReferenceExpression, object data)
        {
            ...
            return _resolver.CallingClass.DefaultReturnType;
        }
        ...
        public override object VisitBaseReferenceExpression(BaseReferenceExpression baseReferenceExpression, object data)
        {
            ...
            return _resolver.CallingClass.BaseType;
        }
        ...
        {
            AnonymousMethodReturnType amrt = new AnonymousMethodReturnType(_resolver.CompilationUnit);
            foreach (ParameterDeclarationExpression param in anonymousMethodExpression.Parameters)
            {
                amrt.MethodParameters.Add(NRefactoryASTConvertVisitor.CreateParameter(param, _resolver.CallingMember as IMethod, _resolver.CallingClass, _resolver.CompilationUnit));
            }
            return amrt;
        }
        ...
        {
            return CreateReturnType(reference, _resolver);
        }
