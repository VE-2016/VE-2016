...
using RefParser = AIMS.Libraries.Scripting.NRefactory;
...
namespace AIMS.Libraries.Scripting.Dom.NRefactoryResolver
{
    public class NRefactoryASTConvertVisitor : AbstractAstVisitor
    {
        ...
        private Stack<string> _currentNamespace = new Stack<string>();
        private Stack<DefaultClass> _currentClass = new Stack<DefaultClass>();
        ...
        {
            _cu = new DefaultCompilationUnit(projectContent);
        }

        private DefaultClass GetCurrentClass()
        {
            return _currentClass.Count == 0 ? null : _currentClass.Peek();
        }
        ...
        private List<RefParser.ISpecial> _specials;
        ...
        private string GetDocumentation(int line, IList<AST.AttributeSection> attributes)
        {
        ...
        {
            if (_specials == null) return null;
            if (line < 0) return null;
            // specials is a sorted list: use interpolation search
            int left = 0;
            int right = _specials.Count - 1;
            int m;

            while (left <= right)
            {
                int leftLine = _specials[left].StartPosition.Y;
                if (line < leftLine)
                    break;
                int rightLine = _specials[right].StartPosition.Y;
                if (line > rightLine)
                    break;
                ...
                int mLine = _specials[m].StartPosition.Y;
                if (mLine < line)
                { // found line smaller than line we are looking for
                    left = m + 1;
                }
                else if (mLine > line)
                {
                ...
                else
                {
                    ...
                    while (--m >= 0 && _specials[m].StartPosition.Y == line) ;
                    // look at all specials in that line: find doku-comment
                    while (++m < _specials.Count && _specials[m].StartPosition.Y == line)
                    {
                        RefParser.Comment comment = _specials[m] as RefParser.Comment;
        ...
        {
            DefaultUsing us = new DefaultUsing(_cu.ProjectContent, GetRegion(usingDeclaration.StartLocation, usingDeclaration.EndLocation));
        ...
        private void ConvertAttributes(AST.AttributedNode from, AbstractDecoration to)
        {
        ...
        private List<IAttribute> VisitAttributes(List<AST.AttributeSection> attributes)
        {
            // TODO Expressions???
        ...
        public override object VisitNamespaceDeclaration(AST.NamespaceDeclaration namespaceDeclaration, object data)
        {
            ...
            {
                name = _currentNamespace.Peek() + '.' + namespaceDeclaration.Name;
            }

            _currentNamespace.Push(name);
            object ret = namespaceDeclaration.AcceptChildren(this, data);
            _currentNamespace.Pop();
            return ret;
        }

        private ClassType TranslateClassType(AST.ClassType type)
        {
            switch (type)
            {
        ...
        private static DomRegion GetRegion(RefParser.Location start, RefParser.Location end)
        {
        ...
        public override object VisitTypeDeclaration(AST.TypeDeclaration typeDeclaration, object data)
        {
            ...
            DefaultClass c = new DefaultClass(_cu, TranslateClassType(typeDeclaration.Type), ConvertTypeModifier(typeDeclaration.Modifier), region, GetCurrentClass());
            ...
            else
            {
                ...
                {
                    c.FullyQualifiedName = _currentNamespace.Peek() + '.' + typeDeclaration.Name;
                }
                _cu.Classes.Add(c);
            }
        ...
        private void ConvertTemplates(IList<AST.TemplateDefinition> templateList, DefaultClass c)
        {
            int index = 0;
        ...
        private void ConvertTemplates(List<AST.TemplateDefinition> templateList, DefaultMethod m)
        {
            int index = 0;
            if (templateList.Count == 0)
            {
        ...
        private DefaultTypeParameter ConvertConstraints(AST.TemplateDefinition template, DefaultTypeParameter typeParameter)
        {
        ...
        public override object VisitDelegateDeclaration(AST.DelegateDeclaration delegateDeclaration, object data)
        {
            ...
            DefaultClass c = new DefaultClass(_cu, ClassType.Delegate, ConvertTypeModifier(delegateDeclaration.Modifier), region, GetCurrentClass());
        ...
        private IParameter CreateParameter(AST.ParameterDeclarationExpression par)
        {
            return CreateParameter(par, null);
        }

        private IParameter CreateParameter(AST.ParameterDeclarationExpression par, IMethod method)
        {
            return CreateParameter(par, method, GetCurrentClass(), _cu);
        }
        ...
        private bool IsVisualBasic
        {
            get
            {
                return _cu.ProjectContent.Language == LanguageProperties.VBNet;
            }
        }
                ...
                for (int i = 0; i < fieldDeclaration.Fields.Count; ++i)
                {
                    ...
                    else
                    {
                        ...
                        if (!field.FixedArrayInitialization.IsNull)
                            retType = new ArrayReturnType(_cu.ProjectContent, retType, 1);
                    }
        ...
        public override object VisitEventDeclaration(AST.EventDeclaration eventDeclaration, object data)
        {
            ...
            {
                DefaultClass del = new DefaultClass(_cu, ClassType.Delegate,
